...
function testMock (fileName) {
    ...
    test('testing ' + fileName, function (t) {
        ...
        var savedImagePath = 'test/test-images/' + imageFileName;
        var savedImageStream = fs.createWriteStream(savedImagePath);
        var options = getOptions(bodyMock, 'http://local.plot.ly:9010/');
        ...
        function checkImage () {
            var diffPath = 'test/test-images-diffs/' + 'diff-' + imageFileName;
            var options = {
                file: diffPath,
                highlightColor: 'purple',
                tolerance: 0.0
            };
            gm.compare(savedImagePath, 'test/test-images-baseline/' + imageFileName, options, onEqualityCheck);
...
function getOptions (body, url) {
    ...
    var opts = {
        url: url || 'http://local.plot.ly:9010/',
        method: 'POST'
    };
