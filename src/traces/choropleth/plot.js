...
var getTopojsonFeatures = require(
  "../../lib/topojson_utils"
).getTopojsonFeatures;
var locationToFeature = require(
  "../../lib/geo_location_utils"
).locationToFeature;
...
plotChoropleth.calcGeoJSON = function(trace, topojson) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    // filter the blank features here
...
plotChoropleth.plot = function(geo, calcData, geoLayout) {
  ...
  gChoroplethTraces.each(function(calcTrace) {
    ...
    function handleMouseOver(pt, ptIndex) {
      ...
      Fx.loneHover(
        { x: xy[0], y: xy[1], name: pt.nameLabel, text: pt.textLabel },
