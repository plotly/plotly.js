...
function makePath(pi, loc, edgeflag) {
  ...
  for (cnt = 0; cnt < 10000; cnt++) {
    ...
    if (mi > 20) {
      mi = constants.CHOOSESADDLE[mi][
        (marchStep[0] || marchStep[1]) < 0 ? 0 : 1
      ];
    ...
    var atEdge = marchStep[0] && (loc[0] < 0 || loc[0] > n - 2) ||
      ...
      closedLoop = locStr === startLocStr &&
        marchStep.join(",") === startStepStr;
  ...
  for (cnt = pts.length - 2; cnt >= cropstart; cnt--) {
    ...
    if (distgroup < distThreshold) {
      ...
      if (!closedpath && cnt === pts.length - 2) {
        ...
        ptcnt % 2 // odd # of points - just take the central one
      ) {
        newpt = getpt(ptavg);
      } else {
    ...
    pi.edgepaths.forEach(function(edgepath, edgei) {
      if (!merged && equalPts(edgepath[0], pts[pts.length - 1])) {
        ...
        pi.edgepaths.forEach(function(edgepath2, edgei2) {
          if (
            !doublemerged && equalPts(edgepath2[edgepath2.length - 1], pts[0])
          ) {
            doublemerged = true;
            pts.splice(0, 1);
