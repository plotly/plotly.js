    ...
    d3
      ...
      .each(function(di, i) {
        ...
        var lw = (di.mlw + 1 ||
          ...
          (di.trace ? di.trace.marker.line.width : 0) + 1) -
          1,
        ...
        function expandToVisible(v, vc) {
          ...
          return Math.abs(v - vc) >= 2
            ? roundWithLine(v) // but if it's very thin, expand it so it's // necessarily visible, even if it might overlap // its neighbor
...
function appendBarText(gd, bar, calcTrace, i, x0, x1, y0, y1) {
  ...
  if (textPosition === "auto") {
    if (isOutmostBar) {
      ...
      textBB = Drawing.bBox(
        textSelection.node()
      ), textWidth = textBB.width, textHeight = textBB.height;
      ...
      if (
        textHasSize && (fitsInside || fitsInsideIfRotated || fitsInsideIfShrunk)
      ) {
        textPosition = "inside";
      } else {
