...
function ScatterMapLibre(subplot, uid, clusterEnabled, isHidden) {
    this.type = 'scattermaplibre';
    ...
    // for up-to 4 sources per 'scattermaplibre' traces becomes a problem.
...
var proto = ScatterMapLibre.prototype;
...
module.exports = function createScatterMapLibre(subplot, calcTrace) {
    var trace = calcTrace[0].trace;
    var hasCluster = trace.cluster && trace.cluster.enabled;
    var isHidden = trace.visible !== true;

    var scatterMapLibre = new ScatterMapLibre(
        subplot,
        trace.uid,
        hasCluster,
        isHidden
    ...
    var optsAll = convert(subplot.gd, calcTrace);
    var below = scatterMapLibre.below = subplot.belowLookup['trace-' + trace.uid];
    var i, k, opts;

    if(hasCluster) {
        scatterMapLibre.addSource('circle', optsAll.circle, trace.cluster);
        for(i = 0; i < ORDER.cluster.length; i++) {
            k = ORDER.cluster[i];
            opts = optsAll[k];
            scatterMapLibre.addLayer(k, opts, below);
        }
    } else {
        for(i = 0; i < ORDER.nonCluster.length; i++) {
            k = ORDER.nonCluster[i];
            opts = optsAll[k];
            scatterMapLibre.addSource(k, opts, trace.cluster);
            scatterMapLibre.addLayer(k, opts, below);
        }
    }

    // link ref for quick update during selections
    calcTrace[0].trace._glTrace = scatterMapLibre;

    return scatterMapLibre;
};
