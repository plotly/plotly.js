...
proto.handlePick = function(selection) {
  if (
    ...
    if (
      this.textLabels && this.textLabels[selection.data.index] !== undefined
    ) {
...
function constructDelaunay(points, color, axis) {
  ...
  return { positions: points, cells: cells, meshColor: color };
}
...
function formatParam(paramIn, len, calculate, dflt, extraFn) {
  ...
  if (Array.isArray(paramIn)) {
    ...
    paramOut = calculate(paramIn, Lib.identity);
  }
...
function convertPlotlyOptions(scene, data) {
  ...
  params = { position: points, mode: data.mode, text: text };
