...
module.exports = function(root, svg, styledData, layout, callbacks) {
    ...
    var sankeyLineLayers = root.selectAll('.sankey-line-layers')
        .data(vm, keyFun);

    sankeyLineLayers.exit().remove();

    sankeyLineLayers.enter()
        .insert('div', '.' + svg.attr('class').split(' ').join(' .')) // not hardcoding .main-svg
        .classed('sankey-line-layers', true)
    ...
    sankeyLineLayer.enter()
        .append('canvas')
        .attr('class', function(d) {return 'sankey-lines ' + (d.context ? 'context' : d.pick ? 'pick' : 'focus');})
        .style('box-sizing', 'content-box')
        .style('float', 'left')
        .style('clear', 'both')
        .style('left', 0)
    ...
    function updatePanelLayoutSankey(yAxis, vm) {
    ...
    function updatePanelLayout(yAxis, vm) {
        return (c.scatter ? updatePanelLayoutScatter : updatePanelLayoutSankey)(yAxis, vm);
    }
