    ...
    pieGroup.selectAll(".top").each(function() {
      ...
      slices.each(function(pt) {
        ...
        function handleMouseOver(evt) {
          ...
          if (
            trace2.text && trace2.text[pt.i] && hoverinfo.indexOf("text") !== -1
          ) {
        ...
        function handleMouseOut(evt) {
          gd.emit("plotly_unhover", { points: [evt] });
        ...
        if (trace.pull) {
          var pull = +(Array.isArray(trace.pull)
            ? trace.pull[pt.i]
            : trace.pull) ||
            0;
          if (pull > 0) {
        ...
        sliceTextGroup.each(function() {
          ...
          sliceText.enter().append("text").attr("data-notex", 1);
          ...
          var translateX = cx +
            pt.pxmid[0] * transform.rCenter +
            (transform.x || 0),
            translateY = cy +
              pt.pxmid[1] * transform.rCenter +
              (transform.y || 0);
      ...
      slices.each(function(pt) {
        if (pt.labelExtraX || pt.labelExtraY) {
          ...
          var lineStartX = pt.cxFinal + pt.pxmid[0],
            ...
            finalX = (pt.yLabelMax - pt.yLabelMin) *
              (pt.pxmid[0] < 0 ? -1 : 1) /
              4;
          if (pt.labelExtraX) {
...
function transformInsideText(textBB, pt, cd0) {
  ...
  var Qr = textAspect + 1 / (2 * Math.tan(halfAngle)),
    ...
    radialTransform = {
      ...
      rCenter: (
        Math.cos(maxHalfHeightRotRadial / cd0.r) -
    ...
    tangentialTransform = {
      ...
      rCenter: (
        Math.cos(maxHalfWidthTangential / cd0.r) -
...
function scootLabels(quadrants, trace) {
  ...
  function scootOneLabel(thisPt, prevPt) {
    ...
    var prevOuterY = prevPt.labelExtraY +
      (yHalf ? prevPt.yLabelMax : prevPt.yLabelMin),
      thisInnerY = yHalf ? thisPt.yLabelMin : thisPt.yLabelMax,
      thisOuterY = yHalf ? thisPt.yLabelMax : thisPt.yLabelMin,
      thisSliceOuterY = thisPt.cyFinal +
        farthestY(thisPt.px0[1], thisPt.px1[1]),
    ...
    if (!Array.isArray(trace.pull)) return;

    // this can only happen with array pulls
    for (i = 0; i < wholeSide.length; i++) {
      ...
      if ((thisPt.pxmid[1] - otherPt.pxmid[1]) * yDiffSign > 0) {
        ...
        otherOuterY = otherPt.cyFinal +
          farthestY(otherPt.px0[1], otherPt.px1[1]);
        newExtraY = otherOuterY - thisInnerY - thisPt.labelExtraY;

        if (newExtraY * yDiffSign > 0) thisPt.labelExtraY += newExtraY;
      } else if (
        (thisOuterY + thisPt.labelExtraY - thisSliceOuterY) * yDiffSign > 0
      ) {
        ...
        otherOuterX = otherPt.cxFinal +
          farthestX(otherPt.px0[0], otherPt.px1[0]);
        newExtraX = otherOuterX +
          xBuffer -
...
function scalePies(cdpie, plotSize) {
  ...
  for (i = 0; i < cdpie.length; i++) {
    ...
    cd0.cx = plotSize.l +
      plotSize.w * (trace.domain.x[1] + trace.domain.x[0]) / 2;
    cd0.cy = plotSize.t +
      plotSize.h * (2 - trace.domain.y[1] - trace.domain.y[0]) / 2;
...
function maxExtent(tilt, tiltAxisFraction, depth) {
  ...
  return Math.max(
    0.01,
    // don't let it go crazy if you tilt the pie totally on its side
