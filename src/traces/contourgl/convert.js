...
function convertColorScale(fullTrace, options) {
  ...
  var N = Math.floor((end - start) / cs) + (fill ? 2 : 1),
    // for K thresholds (contour linees) there are K+1 areas
    levels = new Array(N),
    levelColors = new Array(4 * N);

  for (var i = 0; i < N; i++) {
    var level = levels[i] = start + cs * i - (fill ? cs / 2 : 0);
    // in case of fill, use band midpoint
  ...
  return { levels: levels, levelColors: levelColors };
}
