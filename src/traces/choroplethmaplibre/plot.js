...
function ChoroplethMapLibre(subplot, uid) {
    this.type = 'choroplethmaplibre';
...
var proto = ChoroplethMapLibre.prototype;
...
module.exports = function createChoroplethMapLibre(subplot, calcTrace) {
    var trace = calcTrace[0].trace;
    var choroplethMapLibre = new ChoroplethMapLibre(subplot, trace.uid);
    var sourceId = choroplethMapLibre.sourceId;
    var optsAll = convert(calcTrace);
    var below = choroplethMapLibre.below = subplot.belowLookup['trace-' + trace.uid];
    ...
    choroplethMapLibre._addLayers(optsAll, below);

    // link ref for quick update during selections
    calcTrace[0].trace._glTrace = choroplethMapLibre;

    return choroplethMapLibre;
};
