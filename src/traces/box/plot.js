...
var JITTERCOUNT = 5,
  // points either side of this to include
  JITTERSPREAD = 0.01;

// fraction of IQR to count as "dense"
      ...
      d3
        ...
        .data(function(d) {
          ...
          return pts.map(function(v, i) {
            ...
            if (trace.orientation === "h") {
              p = { y: d.pos + posOffset * bdPos + bPos, x: v };
            } else {
              p = { x: d.pos + posOffset * bdPos + bPos, y: v };
            }

            // tag suspected outliers
            if (
              trace.boxpoints === "suspectedoutliers" && v < d.uo && v > d.lo
            ) {
              p.so = true;
            }
            return p;
