...
module.exports = function calc(gd, trace) {
  ...
  val = valAxis.makeCalcdata(trace, valLetter);
  ...
  function getPos(gd, trace, posLetter, posAxis, val) {
    ...
    if (posLetter in trace) {
      pos = posAxis.makeCalcdata(trace, posLetter);
    } else {
  ...
  function calculateStats(cd, valBinned) {
    ...
    for (i = 0; i < valBinned.length; ++i) {
      ...
      cdi.val = v;
      // put all values into calcdata
      cdi.min = v[0];
      cdi.max = v[l - 1];
      cdi.mean = Lib.mean(v, l);
      cdi.sd = Lib.stdev(v, l, cdi.mean);
      cdi.q1 = Lib.interp(v, 0.25);
      // first quartile
      cdi.med = Lib.interp(v, 0.5);
      // median
      cdi.q3 = Lib.interp(v, 0.75);
      // third quartile
