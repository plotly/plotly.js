...
module.exports = function convert(calcTrace) {
  ...
  var opts = { fill: fill, line: line, circle: circle, symbol: symbol };
  ...
  if (hasFill) {
    ...
    Lib.extendFlat(fill.paint, { "fill-color": trace.fillcolor });
  }
  ...
  if (hasSymbols || hasText) {
    ...
    if (hasSymbols) {
      Lib.extendFlat(symbol.layout, { "icon-size": trace.marker.size / 10 });
...
function makeCircleGeoJSON(calcTrace, hash) {
  ...
  for (var i = 0; i < calcTrace.length; i++) {
    ...
    features.push({
      type: "Feature",
      geometry: { type: "Point", coordinates: calcPt.lonlat },
      properties: props
    });
  }
...
function makeSymbolGeoJSON(calcTrace) {
  ...
  for (var i = 0; i < calcTrace.length; i++) {
    ...
    features.push({
      type: "Feature",
      geometry: { type: "Point", coordinates: calcPt.lonlat },
      properties: { symbol: fillSymbol(calcPt.mx), text: fillText(calcPt.tx) }
    });
  }
...
function calcCircleColor(trace, hash) {
  ...
  if (Array.isArray(marker.color)) {
    ...
    out = { property: COLOR_PROP, stops: stops };
  } else {
    out = marker.color;
  }
...
function calcCircleRadius(trace, hash) {
  ...
  if (Array.isArray(marker.size)) {
    ...
    out = { property: SIZE_PROP, stops: stops };
  } else {
    out = marker.size / 2;
  }
