...
function createHoverText(hoverData, opts) {
  ...
  hoverLabels.each(function(d) {
    ...
    g.select("path").style({ fill: traceColor, stroke: contrastColor });
...
function alignHoverText(hoverLabels, rotateLabels) {
  ...
  hoverLabels.each(function(d) {
    ...
    g.select("path").attr(
      "d",
      d.anchor === "middle" // middle aligned: rect centered on data
        ? "M-" +
            d.bx / 2 +
            ",-" +
            d.by / 2 +
            ...
            "h-" +
            d.bx +
            "Z" // left or right aligned: side rect with arrow to data
        : "M0,0L" +
