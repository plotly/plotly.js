...
function findCategoryPairs(ax, opts) {
    var dataAttr = opts.dataAttr || ax._id.charAt(0);
    var axData = getAxData(ax, opts);
    // prototype-less, so that e.g. a category named 'toString' does not
    ...
    for(i = 0; i < axData.length; i++) {
        ...
        for(j = 0; j < len; j++) {
            ...
            if(isValidCategory(v0) && isValidCategory(v1)) {
                // comma-joined keys match how `_categoriesMap` stringifies
                // [parent, child] pairs in set_convert.js
                var key = v0 + ',' + v1;
                if(!(key in lookup)) {
                    lookup[key] = 1;
                    list.push([v0, v1]);
...
module.exports = function handleCategoryOrderDefaults(containerIn, containerOut, coerce, opts) {
    ...
    var order;
    if(isMultiCategory) {
        // ordering by aggregated value ('total ascending', 'median descending', ...)
        // is not implemented for multicategory axes - it would interleave
        // children across different parents, breaking the parent grouping -
        // so coerce against the modes that are
        order = Lib.coerce(containerIn, containerOut, {
            categoryorder: {
                valType: 'enumerated',
                values: ['trace', 'category ascending', 'category descending', 'array'],
                dflt: 'trace'
            }
        }, 'categoryorder', orderDefault);
    } else {
        order = coerce('categoryorder', orderDefault);
    }
    var array;
