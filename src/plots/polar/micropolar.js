...
µ.Axis = function module() {
  ...
  function render(_container) {
    ...
    container.datum(data).each(function(_data, _index) {
      ...
      var useProvidedDomain = axisConfig.angularAxis.domain &&
      ...
      if (axisConfig.angularAxis.ticksStep) {
        tickCount = (angularDomainWithPadding[1] -
          angularDomainWithPadding[0]) /
          tickCount;
      }
      ...
      if (axisConfig.showLegend) {
        ...
        var elements = data.map(function(d, i) {
          ...
          datumClone.visibleInLegend = typeof d.visibleInLegend ===
            "undefined" ||
            d.visibleInLegend;
      ...
      svg
        .attr({ width: axisConfig.width, height: axisConfig.height })
      ...
      if (axisConfig.radialAxis.visible) {
        ...
        radialAxis.selectAll("g>line").style({ stroke: "black" });
      }
      var angularAxis = svg
      ...
      angularAxisEnter
        .selectAll(".minor")
        .style({ stroke: axisConfig.minorTickColor });
      angularAxis
      ...
      var hasGeometry = svg.select("g.geometry-group").selectAll("g").size() >
        0;
      ...
      var angularTooltip = µ
        .tooltipPanel()
        .config({ container: tooltipContainer, fontSize: 8 })();
      var radialTooltip = µ
        .tooltipPanel()
        .config({ container: tooltipContainer, fontSize: 8 })();
      var geometryTooltip = µ
        .tooltipPanel()
        .config({ container: tooltipContainer, hasTick: true })();
      ...
      if (!isOrdinal) {
        ...
        chartGroup
          .on("mousemove.angular-guide", function(d, i) {
            ...
            angularGuideLine
              .attr({ x2: -radius, transform: "rotate(" + mouseAngle + ")" })
              .style({ opacity: 0.5 });
            var angleWithOriginOffset = (mouseAngle +
              180 +
              360 -
          ...
          .on("mouseout.angular-guide", function(d, i) {
            guides.select("line").style({ opacity: 0 });
          });
      }
      var angularGuideCircle = guides
        .select("circle")
      ...
      svg
        ...
        .on("mouseout.tooltip", function(d, i) {
          ...
          if (fillColor) {
            el.style({ fill: fillColor, opacity: el.attr("data-opacity") });
          } else {
            el.style({
...
µ.PolyChart = function module() {
  ...
  function exports() {
    ...
    container.datum(config).each(function(_config, _index) {
      ...
      var triangleAngle = Math.abs(angularRange[1] - angularRange[0]) /
        data[0].length *
        Math.PI /
        180;
      var arc = d3.svg
      ...
      function getPolarCoordinates(d, i) {
        ...
        var t = (geometryConfig.angularScale(d[0]) +
          geometryConfig.orientation) *
          Math.PI /
          180;
        return { r: r, t: t };
      }
...
µ.BarChart.defaultConfig = function() {
  var config = { geometryConfig: { geometryType: "bar" } };
  return config;
};
...
µ.AreaChart.defaultConfig = function() {
  var config = { geometryConfig: { geometryType: "arc" } };
  return config;
};
...
µ.DotPlot.defaultConfig = function() {
  var config = { geometryConfig: { geometryType: "dot", dotType: "circle" } };
  return config;
};
...
µ.LinePlot.defaultConfig = function() {
  var config = { geometryConfig: { geometryType: "line" } };
  return config;
};
...
µ.Legend = function module() {
  ...
  function exports() {
    ...
    data = data.filter(function(d, i) {
      return legendConfig.elements[i] &&
    ...
    var dataScale = d3.scale
      [isContinuous ? "linear" : "ordinal"]()
      .domain(dataNumbered)
      [isContinuous ? "range" : "rangePoints"]([0, height]);
    ...
    if (isContinuous) {
      ...
      legendElement.attr({
        transform: function(d, i) {
          return "translate(" +
            [lineHeight / 2, dataScale(i) + lineHeight / 2] +
            ")";
        },
        d: function(d, i) {
...
µ.tooltipPanel = function() {
  ...
  var exports = function() {
    ...
    backgroundEl = tooltipEnter
      .append("path")
      .style({ fill: "white", "fill-opacity": 0.9 })
      .attr({ d: "M0 0" });
    tooltipTextEl = tooltipEnter
      .append("text")
  ...
  exports.text = function(_text) {
    ...
    var text = _text || "";
    tooltipTextEl
      .style({ fill: fillColor, "font-size": config.fontSize + "px" })
      .text(text);
    ...
    tooltipEl.attr({
      transform: (
        "translate(" + [tickSize, (-backGroundH) / 2 + padding * 2] + ")"
      )
    });
  ...
  exports.move = function(_pos) {
    if (!tooltipEl) return;
    tooltipEl
      .attr({ transform: "translate(" + [_pos[0], _pos[1]] + ")" })
      .style({ display: "block" });
    return exports;
  };
