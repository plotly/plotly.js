...
plots.addLinks = function(gd) {
  ...
  var text = linkContainer.node(),
    attrs = { y: fullLayout._paper.attr("height") - 9 };
...
plots.sendDataToCloud = function(gd) {
  ...
  var baseUrl = window.PLOTLYENV && window.PLOTLYENV.BASE_URL ||
    "https://plot.ly";
  ...
  var hiddenform = hiddenformDiv
    .append("form")
    .attr({ action: baseUrl + "/external", method: "post", target: "_blank" });
...
function relinkPrivateKeys(toContainer, fromContainer) {
  var isPlainObject = Lib.isPlainObject, isArray = Array.isArray;
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (k.charAt(0) === "_" || typeof fromVal === "function") {
      ...
      isArray(fromVal) && isArray(toVal) && isPlainObject(fromVal[0])
    ) {
...
plots.linkSubplots = function(
  ...
  var mockGd = { _fullData: newFullData, _fullLayout: newFullLayout };
...
plots.plotAutoSize = function plotAutoSize(gd, layout, fullLayout) {
  ...
  if (context.fillFrame) {
    ...
    var gdBB = fullLayout._container && fullLayout._container.node
      ...
      : { width: fullLayout.width, height: fullLayout.height };
...
plots.purge = function(gd) {
  ...
  delete gd.undonum;
  delete gd.autoplay;
  // are we doing an action that doesn't go in undo queue?
  delete gd.changed;
...
plots.autoMargin = function(gd, id, o) {
  ...
  if (fullLayout.margin.autoexpand !== false) {
    if (!o) {
      delete fullLayout._pushmargin[id];
    } else {
        ...
        if (isNumeric(pl) && pm[k2].r) {
          ...
          if (fr > fl) {
            var newl = (pl * fr + (pr - fullLayout.width) * fl) / (fr - fl),
              newr = (pr * (1 - fl) + (pl - fullLayout.width) * (1 - fr)) /
                (fr - fl);
        ...
        if (isNumeric(pb) && pm[k2].t) {
          ...
          if (ft > fb) {
            var newb = (pb * ft + (pt - fullLayout.height) * fb) / (ft - fb),
              newt = (pt * (1 - fb) + (pb - fullLayout.height) * (1 - ft)) /
                (ft - fb);
  ...
  function stripObj(d) {
    ...
    if (Lib.isPlainObject(d)) {
      ...
      for (v in d) {
        ...
        if (
          typeof d[v] === "function" || ["_", "["].indexOf(v.charAt(0)) !== -1
        ) {
          continue;
        }
      ...
      if (layout) {
        for (j = 0; j < basePlotModules.length; j++) {
          if (basePlotModules[j].transitionAxes) {
            ...
            hasAxisTransition = basePlotModules[j].transitionAxes(
              gd,
              newLayout,
              transitionOpts,
              makeCallback
