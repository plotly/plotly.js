...
var createMapLibreLayer = require('./layers');

function MapLibre(gd, id) {
    this.id = id;
    this.gd = gd;
...
proto.updateData = function(calcData) {
    ...
    // e.g. choroplethmaplibre traces will be below scattermaplibre traces
...
proto.updateLayers = function(fullLayout) {
    ...
    if(layers.length !== layerList.length) {
        ...
        for(i = 0; i < layers.length; i++) {
            layerList.push(createMapLibreLayer(this, i, layers[i]));
        }
    } else {
        for(i = 0; i < layers.length; i++) {
