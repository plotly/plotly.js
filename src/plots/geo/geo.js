...
proto.plot = function(geoCalcData, fullLayout, promises) {
  ...
  _this.framework.on("mousemove", function() {
    ...
    var evt = { target: true, xpx: mouse[0], ypx: mouse[1] };
...
proto.makeFramework = function() {
  ...
  geoDiv.attr("id", this.id).style("position", "absolute");
  ...
  hoverContainer
    ...
    .style({ position: "absolute", "z-index": 20, "pointer-events": "none" });

  var framework = this.framework = geoDiv.append("svg");
  framework
    .attr(xmlnsNamespaces.svgAttrs)
    .attr({ position: "absolute", preserveAspectRatio: "none" });
...
proto.adjustLayout = function(geoLayout, graphSize) {
  ...
  this.framework.attr({ width: geoLayout._width, height: geoLayout._height });

  this.framework
    .select(".bglayer")
    .select("rect")
    .attr({ width: geoLayout._width, height: geoLayout._height })
...
proto.drawLayout = function(geoLayout) {
  ...
  for (var i = 0; i < baseLayers.length; i++) {
    ...
    if (axesNames.indexOf(layerName) !== -1) {
      ...
      this.drawTopo(gBaseLayer, layerName, geoLayout);
    }
  }
