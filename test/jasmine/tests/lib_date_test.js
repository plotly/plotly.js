...
describe("dates", function() {
  ...
  describe("dateTime2ms", function() {
    it("should accept valid date strings", function() {
      ...
      [
        ...
        [
          nowPlus29_2 + "-02-12 14:29:32",
          new Date(nowPlus29, 1, 12, 14, 29, 32)
        ],
        ...
        expect(Lib.dateTime2ms(v[0].trim().replace(" ", "t"))).toBe(
          expected,
          v[0].trim().replace(" ", "t")
        );
        expect(
    ...
    it("should accept 4-digit and 2-digit numbers", function() {
      ...
      [1000, 9999, -1000, -9999].forEach(function(v) {
    ...
    it("should not accept invalid strings or other objects", function() {
      [
        ...
        "2015-01-01Z",
        "2015-01-01T12Z",
        "2015-01-01T12:34Z05:00",
        "2015-01-01 12:34+500",
        // bad TZ info
        "2015-01-01 12:34-5:00"
      ].forEach(function(v) {
        expect(Lib.dateTime2ms(v)).toBeUndefined(v);
      });
    });
    ...
    it(
      "should use UTC with no timezone offset or daylight saving time",
      function() {
    ...
    it(
      "should interpret JS dates by local time, not by its getTime()",
      function() {
  ...
  describe("ms2DateTime", function() {
    it(
      "should report the minimum fields with nonzero values, except minutes",
      function() {
        [
          "2016-01-01",
          // we'll never report less than this bcs month and day are never zero
          "2016-01-01 01:00",
          // we won't report hours without minutes
          "2016-01-01 01:01",
          "2016-01-01 01:01:01",
    ...
    it(
      "should not accept Date objects beyond our limits or other objects",
      function() {
        [
          Date.UTC(10000, 0, 1),
  ...
  describe("world calendar inputs", function() {
    it("should give the right values near epoch zero", function() {
      [
        ...
        expect(Lib.ms2DateTime(oneSec, 0, calendar)).toBe(
          expected_1s,
          calendar
        );
        expect(Lib.ms2DateTime(oneMin, 0, calendar)).toBe(
          expected_1m,
          calendar
        );
        expect(Lib.ms2DateTime(oneHour, 0, calendar)).toBe(
          expected_1h,
          calendar
        );
        expect(Lib.ms2DateTime(lastInstant, 0, calendar)).toBe(
          expected_lastinstant,
          calendar
        );
        ...
        expect(Lib.dateTime2ms(expected_lastinstant, calendar)).toBe(
          lastInstant,
          calendar
        );
      });
    ...
    it(
      "should contain canonical ticks sundays, ranges for all calendars",
      function() {
        ...
        calList.forEach(function(calendar) {
          expect(
            Lib.dateTime2ms(canonicalTick[calendar], calendar)
          ).toBeDefined(calendar);
          ...
          expect(Lib.dateTime2ms(dfltRange[calendar][0], calendar)).toBeDefined(
            calendar
          );
          expect(Lib.dateTime2ms(dfltRange[calendar][1], calendar)).toBeDefined(
            calendar
          );
        });
      }
    ...
    it("should handle Chinese intercalary months correctly", function() {
      ...
      var badIntercalaryDates = ["1995-07i-01", "1995-08i-30", "1995-09i-01"];
  ...
  describe("cleanDate", function() {
    it(
      "should convert numbers or js Dates to strings based on local TZ",
      function() {
        [
          new Date(0),
          new Date(2000),
          ...
          new Date(),
          new Date(-9999, 0, 3),
          // we lose one day of range +/- tzoffset this way
    ...
    it(
      "should fail numbers & js Dates out of range, and other bad objects",
      function() {
        [
          new Date(-20000, 0, 1),
          new Date(20000, 0, 1),
    ...
    it(
      "should not alter valid date strings, even to truncate them",
      function() {
        [
          "2000",
          "2000-01",
  ...
  describe("isJSDate", function() {
    it(
      "should return true for any Date object but not the equivalent numbers",
      function() {
        [
          new Date(),
          new Date(0),
    ...
    it(
      "should return false for anything thats not explicitly a JS Date",
      function() {
        [
          0,
          NaN,
  ...
  describe("formatDate", function() {
    function assertFormatRounds(ms, calendar, results) {
      ["y", "m", "d", "M", "S", 1, 2, 3, 4].forEach(function(tr, i) {
        expect(Lib.formatDate(ms, "", tr, calendar)).toBe(results[i], calendar);
      });
    }

    it(
      "should pick a format based on tickround if no format is provided",
      function() {
    ...
    it(
      "should accept custom formats using d3 specs even for world cals",
      function() {
        var ms = Lib.dateTime2ms("2012-08-13 06:19:34.5678");
        [
          ...
          [
            ...
            // world-cals doesn't support U or w
            "Mesori '28 WOY:## DOW:##"
          ],
          [
            "%c && %x && .%2f .%f",
          ...
          var fmt = v[0], expectedGregorian = v[1], expectedCoptic = v[2];

          // tickround is irrelevant here...
          expect(Lib.formatDate(ms, fmt, "y")).toBe(expectedGregorian, fmt);
          expect(Lib.formatDate(ms, fmt, 4, "gregorian")).toBe(
            expectedGregorian,
            fmt
          );
          expect(Lib.formatDate(ms, fmt, "y", "coptic")).toBe(
            expectedCoptic,
            fmt
          );
        });
    ...
    it("should not round up to 60 seconds", function() {
      ...
      expect(Lib.formatDate(-0.1, "%Y-%m-%d %H:%M:%S.%f")).toBe(
        "1969-12-31 23:59:59.9999"
      );
      expect(Lib.formatDate(-0.1, "%Y-%m-%d %H:%M:%S.%f", null, "coptic")).toBe(
        "1686-04-22 23:59:59.9999"
      );
    });
    ...
    it("should remove extra fractional second zeros", function() {
      ...
      expect(Lib.formatDate(0.1, "", 3, "coptic")).toBe(
        "00:00:00\nKoi 23, 1686"
      );
