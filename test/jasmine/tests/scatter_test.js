...
describe('end-to-end scatter tests', function() {
  ...
  it('should add a plotly-customdata class to points with custom data', function(
    done
  ) {
    Plotly.plot(gd, [
      {
      ...
      .then(function() {
        ...
        points.each(function(cd, i) {
          expect(d3.select(this).classed('plotly-customdata')).toBe(
            expected[i]
          );
        });
...
describe('scatter hoverPoints', function() {
  ...
  it("should show 'hovertext' items when present, 'text' if not", function(
    done
  ) {
