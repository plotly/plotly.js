...
describe("Test scatter", function() {
  ...
  describe("isBubble", function() {
    it("should return true when marker.size is an Array", function() {
      var trace = { marker: { size: [1, 4, 2, 10] } },
  ...
  describe("makeBubbleSizeFn", function() {
    ...
    it(
      "should scale w.r.t. bubble diameter when sizemode=diameter",
      function() {
        trace.marker.sizemode = "diameter";
        sizeFn = makeBubbleSizeFn(trace);

        expected = [0, 0.5, 1.106606605, 0, 50, 500.1066065, 5000000, 0, 0, 0];
    ...
    it("should adjust scaling according to sizeref", function() {
      ...
      expected = [0, 5, 11.06606605, 0, 500, 5001.066065, 50000000, 0, 0, 0];
      expect(markerSizes.map(sizeFn)).toEqual(expected);
    });
    ...
    it("should adjust the small sizes according to sizemin", function() {
      ...
      expected = [0, 5, 5, 0, 5, 50.01066065, 500000, 0, 0, 0];
      expect(markerSizes.map(sizeFn)).toEqual(expected);
    });
  });
  ...
  describe("linePoints", function() {
    ...
    it("should separate out blanks, unless connectgaps is true", function() {
      ...
      expect(ptsDisjoint).toEqual([
        [[0, 0], [10, 20], [20, 10]],
        [[30, 40]],
        [[40, 60], [50, 30]]
      ]);
      expect(ptsConnected).toEqual([
        [[0, 0], [10, 20], [20, 10], [30, 40], [40, 60], [50, 30]]
      ]);
    });
    ...
    it("should collapse a vertical cluster into 4 points", function() {
      ...
      expect(ptsOut).toEqual([
        [[-10, 0], [0, 0], [0, 10], [0, 20], [0, -25], [0, 5], [10, 10]]
      ]);
      ...
      expect(ptsOut).toEqual([
        [[-10, 0], [0, 0], [0, 10], [0, -25], [0, 20], [0, 5], [10, 10]]
      ]);
    });
    ...
    it("should collapse a horizontal cluster into 4 points", function() {
      ...
      expect(ptsOut).toEqual([
        [[0, -10], [0, 0], [10, 0], [20, 0], [-25, 0], [5, 0], [10, 10]]
      ]);
      ...
      expect(ptsOut).toEqual([
        [[0, -10], [0, 0], [10, 0], [-25, 0], [20, 0], [5, 0], [10, 10]]
      ]);
    });

    it(
      "should use lineWidth to determine whether a cluster counts",
      function() {
        var ptsIn = [
          [0, 0],
          [20, 0],
        ...
        expect(ptsThick).toEqual([
          [[0, 0], [20, 0], [21, 10], [22, 20], [25, -25], [27, 5], [100, 10]]
        ]);
      }
    );
