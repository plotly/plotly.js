...
describe("Layout images", function() {
  describe("supplyLayoutDefaults", function() {
    ...
    it("should reject when there is no `source`", function() {
      ...
      expect(layoutOut.images).toEqual([
        { visible: false, _index: 0, _input: layoutIn.images[0] }
      ]);
    });
  ...
  describe("drawing", function() {
    ...
    it("should draw images on the right layers", function() {
      ...
      Plotly.plot(gd, data, {
        images: [{ source: "imageabove", layer: "above" }]
      });
      ...
      gd = createGraphDiv();
      Plotly.plot(gd, data, {
        images: [{ source: "imagebelow", layer: "below" }]
      });
      ...
      Plotly.plot(gd, data, {
        images: [
          { source: "imagesubplot", layer: "below", xref: "x", yref: "y" }
        ]
      });
  ...
  describe("when relayout", function() {
    ...
    beforeEach(function(done) {
      gd = createGraphDiv();
      Plotly.plot(gd, data, {
        images: [{ source: jsLogo, x: 2, y: 2, sizex: 1, sizey: 1 }],
        width: 500,
        height: 400
      }).then(done);
    });
    ...
    it("should update the image if changed", function(done) {
      var img = Plotly.d3.select("image"), url = img.attr("xlink:href");
    ...
    it("should update the image position if changed", function(done) {
      var update = { "images[0].x": 0, "images[0].y": 1 };
  ...
  describe("when adding/removing images", function() {
    ...
    it("should properly add and removing image", function(done) {
      ...
      function makeImage(source, x, y) {
        return { source: source, x: x, y: y, sizex: 1, sizey: 1 };
      }
