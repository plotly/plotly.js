...
describe('images log/linear axis changes', function() {
  ...
  it('doesnt try to update position automatically with ref changes', function(
    done
  ) {
    ...
    Plotly.relayout(gd, { 'images[0].yref': 'y2' })
      ...
      .then(function() {
        ...
        expect(d3.select('image').attr('clip-path') || '').toMatch(
          /^[^x]+y2\)$/
        );
  ...
  it('keeps the same data value if the axis type is changed without position', function(
    done
  ) {
