...
describe("update menus defaults", function() {
  ...
  it("should default 'bgcolor' to layout 'paper_bgcolor'", function() {
    var buttons = [{ method: "relayout", args: ["title", "Hello World"] }];

    layoutIn.updatemenus = [
      { buttons: buttons },
      { bgcolor: "red", buttons: buttons }
    ];
...
describe("update menus interactions", function() {
  ...
  it("should draw only visible menus", function(done) {
    ...
    Plotly.relayout(gd, "updatemenus[0].visible", false)
      ...
      .then(function() {
        ...
        return Plotly.relayout(gd, {
          "updatemenus[2]": {
            buttons: [{ method: "relayout", args: ["title", "new title"] }]
          }
        });
      })
      .then(function() {
  ...
  it("should drop/fold buttons when clicking on header", function(done) {
    var header0 = selectHeader(0), header1 = selectHeader(1);
  ...
  it("should update correctly on failed binding comparisons", function(done) {
    ...
    // for more info.
    var data = [
      { y: [1, 2, 3], visible: true },
      { y: [2, 3, 1], visible: false },
      { y: [3, 1, 2], visible: false }
    ];
  ...
  it("should change color on mouse over", function(done) {
    var INDEX_0 = 2, INDEX_1 = gd.layout.updatemenus[1].active;
    ...
    Plotly.relayout(gd, "updatemenus[0].bgcolor", "red")
      ...
      .then(function() {
        ...
        return Plotly.relayout(
          gd,
          "updatemenus[1].buttons[1].label",
          "a looooooooooooong<br>label"
        );
      })
      .then(function() {
  ...
  it("applies y padding on relayout", function(done) {
    ...
    Plotly.relayout(gd, { "updatemenus[0].x": 0.5, "updatemenus[0].pad.r": 0 })
      .then(function() {
        // Extract the x-component of the translation:
        x1 = parseInt(
          firstMenu.attr("transform").match(/translate\(([^,]*).*/)[1]
        );
  ...
  function assertItemDims(node, width, height) {
    var rect = node.select("rect"), actualWidth = +rect.attr("width");
...
describe("update menus interaction with other components:", function() {
  ...
  it("buttons show be drawn above sliders", function(done) {
    Plotly.plot(createGraphDiv(), [{ x: [1, 2, 3], y: [1, 2, 1] }], {
      sliders: [
        {
          xanchor: "right",
          x: -0.05,
...
describe("update menus interaction with scrollbox:", function() {
  "use strict";
  var gd, mock, menuDown, menuLeft, menuRight, menuUp;
