...
describe("svg+text utils", function() {
  ...
  describe("convertToTspans should", function() {
    ...
    it(
      "check for XSS attack in href (with plenty of white spaces)",
      function() {
        var node = mockTextSVGElement(
          "<a href =    \"     javascript:alert('attack')\">XSS</a>"
        );
    ...
    it("whitelist relative hrefs (interpreted as http)", function() {
      var node = mockTextSVGElement('<a href="/mylink">mylink</a>');
    ...
    it("whitelist https hrefs", function() {
      var node = mockTextSVGElement('<a href="https://plot.ly">plot.ly</a>');
    ...
    it("wrap XSS attacks in href", function() {
      ...
      textCases.forEach(function(textCase) {
        ...
        assertAnchorLink(
          node,
          "XSS onmouseover=alert(1) style=font-size:300px"
        );
      });
    });
    ...
    it("should keep query parameters in href", function() {
      ...
      textCases.forEach(function(textCase) {
        ...
        assertAnchorLink(
          node,
          "https://abc.com/myFeature.jsp?name=abc&pwd=def"
        );
      });
    });

    it("allow basic spans", function() {
      var node = mockTextSVGElement("<span>text</span>");
    ...
    it("ignore unquoted styles in spans", function() {
      var node = mockTextSVGElement("<span style=unquoted>text</span>");
    ...
    it("allow quoted styles in spans", function() {
      var node = mockTextSVGElement('<span style="quoted: yeah;">text</span>');
    ...
    it("decode some HTML entities in text", function() {
      ...
      expect(node.text()).toEqual(
        "100\u03BC & < 10 > 0 \xA0100 \xD7 20 \xB1 0.5 \xB0"
      );
    });
  });
});
