...
describe("contour defaults", function() {
  ...
  it("should set autocontour to false when contours is supplied", function() {
    ...
    traceIn = {
      ...
      // you need at least start and end
      contours: { start: 4 }
    };
    supplyDefaults(traceIn, traceOut, defaultColor, layout);
...
describe("contour makeColorMap", function() {
  "use strict";
  it(
    "should make correct color map function ('fill' coloring case)",
    function() {
      var trace = {
        contours: { coloring: "fill", start: -1.5, size: 0.5, end: 2.005 },
        colorscale: [
          [0, "rgb(12,51,131)"],
          [0.25, "rgb(10,136,186)"],
          [0.5, "rgb(242,211,56)"],
      ...
      expect(colorMap.domain()).toEqual([-1.75, -0.75, 0.25, 1.25, 2.25]);
  ...
  it(
    "should make correct color map function ('heatmap' coloring case)",
    function() {
      var trace = {
        contours: { coloring: "heatmap", start: 1.5, size: 0.5, end: 5.505 },
      ...
      expect(colorMap.domain()).toEqual([1, 2.75, 3.5, 4, 4.5, 6]);
  ...
  it(
    "should make correct color map function ('lines' coloring case)",
    function() {
      var trace = {
        contours: { coloring: "lines", start: 1.5, size: 0.5, end: 5.505 },
        colorscale: colorScales.RdBu
      };

      var colorMap = makeColorMap(trace);

      expect(colorMap.domain()).toEqual([1.5, 2.9, 3.5, 3.9, 4.3, 5.5]);
...
describe("contour calc", function() {
    ...
    incompleteContours.forEach(function(contoursIn) {
      contoursFinal.forEach(function(spec) {
        ...
        ["start", "end", "size"].forEach(function(attr) {
          expect(out.contours[attr]).toBe(spec[attr], [
            contoursIn,
            spec.inputNcontours,
            attr
          ]);
          // all these get copied back to the input trace
          expect(out._input.contours[attr]).toBe(spec[attr], [
            contoursIn,
            spec.inputNcontours,
            attr
          ]);
  ...
  it(
    "should supply size and reorder start/end if autocontour is off",
    function() {
      var specs = [
      ...
      specs.forEach(function(spec) {
        [
          ...
          var startIn = v[0], endIn = v[1], order = v[2];
