...
describe("ModeBar", function() {
  ...
  var buttons = [
    [{ name: "button 1", click: noop }, { name: "button 2", click: noop }]
  ];
  ...
  describe("createModebar", function() {
    ...
    it("throws when button config does not have a click handler", function() {
      expect(function() {
        createModeBar(getMockGraphInfo(), [[{ name: "not gonna work" }]]);
      }).toThrowError();
    });
  ...
  describe("manageModeBar", function() {
    ...
    it(
      "displays or not mode bar according to displayModeBar config arg",
      function() {
        var gd = getMockGraphInfo();
    ...
    it(
      "updates mode bar buttons if modeBarButtonsToRemove changes",
      function() {
        var gd = setupGraphInfo();
        manageModeBar(gd);
        ...
        expect(countButtons(gd._fullLayout._modeBar)).toEqual(
    ...
    it("updates mode bar buttons if modeBarButtonsToAdd changes", function() {
      ...
      gd._context.modeBarButtonsToAdd = [{ name: "some button", click: noop }];
      manageModeBar(gd);

      expect(countGroups(gd._fullLayout._modeBar)).toEqual(
        initialGroupCount + 1
      );
      expect(countButtons(gd._fullLayout._modeBar)).toEqual(
    ...
    it(
      "sets up buttons with modeBarButtonsToAdd and modeBarButtonToRemove",
      function() {
        var gd = setupGraphInfo();
    ...
    it(
      "sets up buttons with modeBarButtonsToAdd and modeBarButtonToRemove (2)",
      function() {
        var gd = setupGraphInfo();
        gd._context.modeBarButtonsToRemove = [
          "toImage",
    ...
    it("throw error when modeBarButtons contains invalid name", function() {
      var gd = setupGraphInfo();
      gd._context.modeBarButtons = [["toImage", "pan2d", "no gonna work"]];
  ...
  describe("modebar on clicks", function() {
    ...
    function assertActive(buttons, activeButton) {
      for (var i = 0; i < buttons.length; i++) {
        expect(buttons[i].isActive()).toBe(buttons[i] === activeButton);
      }
    }
    ...
    describe("cartesian handlers", function() {
      beforeEach(function(done) {
        ...
        var mockLayout = {
          ...
          yaxis: { anchor: "x", type: "log", range: [1, 3] },
          xaxis2: { anchor: "y2", domain: [0.5, 1], range: [-1, 4] },
      ...
      describe(
        ...
        function() {
          it("should update axis ranges", function() {
            ...
            assertRange("xaxis", [
              "2015-12-27 06:36:39.6661",
              "2016-03-05 17:23:20.3339"
            ]);
    ...
    describe("geo handlers", function() {
      beforeEach(function(done) {
        var mockData = [
          { type: "scattergeo", lon: [10, 20, 30], lat: [10, 20, 30] }
        ];
