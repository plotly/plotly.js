...
var pixelCalc = require('../assets/pixel_calc.js');
...
// Domain referenced shapes tests
// Check shapes are drawn indeed where they should be
// Possibilities: for xref and yref are 'range', 'paper', and 'domain', and each
// can or cannot be a log axis, so there are 36 possibilities x the number of
// shapes.
// The test should be simple: for paper and domain, a shape is drawn in pixels
// based on the chosen plot or axis size and checked against the one produced by
// Plotly. For 'range', data should be produced and the shape checked against
// this data.
// Check that shapes are still drawn where they should be after the domain size
// changes (just change the domain sizes and run the same test).
// Also for each of the above cases, check that we can move the shape properly.
...
//
function checkShapePosition(gd) {}

fdescribe('Test correct shape positions', function() {
    beforeEach(function () {
        gd = createGraphDiv();
        mock = Lib.extendDeep({}, require('@mocks/domain_ref_base.json'));
    });
    ...
    var testCombos = iterable.cartesianProduct([
        axNum,axisTypes,shapePositions,axNum,axisTypes,shapePositions,shapeType
    ]);
        ...
        it('should draw a ' + shapeType
           ...
           + yshapePos.ref,
            function (done) {
                ...
                var mock = Lib.extendDeep({}, require('@mocks/domain_ref_base.json'));
                Plotly.newPlot(gd, mock)
                .then(function() {
                    var shape = {
                        type: shapeType,
                        // this color chosen so it can easily be found with d3
                        line: { color: 'rgb(100, 200, 300)' }
                    };
                ...
                .then(function (shape) {
                    return Plotly.relayout(gd,{shapes: [shape]});
                }).then(function () {
                    checkShapePosition(gd,
                .catch(failTest)
                .then(done);
            }
        });
...
describe('Test shape move', function() {
    beforeEach(function() {
        gd = createGraphDiv();
        mock = Lib.extendDeep({}, require('@mocks/domain_refs_editable.json'));
    });

    // afterAll(function (done) { setTimeout(done,1000); });

    it('should work, no?', function(done) {
        Plotly.newPlot(gd, mock)
        .then(function() {mouseEvent('mousemove', 350, 280);})
        .then(function() {mouseEvent('mousedown', 350, 280);})
        .then(delay(DELAYTIME))
        .then(function() {mouseEvent('mousemove', 350, 200);})
        .then(delay(DELAYTIME))
        .then(function() {mouseEvent('mousemove', 450, 200);})
        .then(function() {
            mouseEvent('mouseup', 450, 200);
            expect(1).toEqual(1);
        })
        .then(delay(DELAYTIME))
        .catch(failTest)
