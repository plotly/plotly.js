...
describe('Test shapes:', function() {
  ...
  describe('each *subplot*', function() {
    ...
    it('should be able to get relayout', function(done) {
      Plotly.relayout(gd, { height: 200, width: 400 })
        .then(function() {
          expect(countShapeLayerNodesInSubplots()).toEqual(countSubplots(gd));
          expect(countShapePathsInSubplots()).toEqual(
            countShapesInSubplots(gd)
          );
        })
        .catch(failTest)
        .then(done);
  ...
  describe('Plotly.relayout', function() {
    ...
    it('can replace the shapes array', function(done) {
      Plotly.relayout(gd, {
        shapes: [getRandomShape(), getRandomShape()],
      })
        .then(function() {
...
describe('shapes axis reference changes', function() {
  ...
  beforeEach(function(done) {
    ...
    Plotly.plot(gd, [{ y: [1, 2, 3] }, { y: [1, 2, 3], yaxis: 'y2' }], {
      yaxis: { domain: [0, 0.4] },
      yaxis2: { domain: [0.6, 1] },
      shapes: [
        {
  ...
  it('draws the right number of objects and updates clip-path correctly', function(
    done
  ) {
...
describe('shapes edge cases', function() {
  ...
  it('falls back on shapeLowerLayer for below missing subplots', function(
    done
  ) {
    Plotly.newPlot(
      gd,
...
describe('Test shapes: a plot with shapes and an overlaid axis', function() {
  ...
  it('should not throw an exception', function(done) {
    Plotly.plot(gd, data, layout).catch(failTest).then(done);
  });
});
...
describe('Test shapes', function() {
  ...
  testCases.forEach(function(testCase) {
    ['n', 's', 'w', 'e', 'nw', 'se', 'ne', 'sw'].forEach(function(direction) {
      var testTitle =
        testCase.title + 'should be resizeable over direction ' + direction;
