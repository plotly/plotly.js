...
describe("Test shapes defaults:", function() {
  ...
  it("should make non-object item visible: false", function() {
    ...
    out.forEach(function(item, i) {
      expect(item).toEqual({ visible: false, _input: {}, _index: i });
    });
  });
  ...
  it("should provide the right defaults on all axis types", function() {
    ...
    var layoutIn = { shapes: [shape1In, shape2In] };

    _supply(layoutIn, fullLayout);

    var shape1Out = fullLayout.shapes[0], shape2Out = fullLayout.shapes[1];
...
describe("Test shapes:", function() {
  ...
  describe("*shapeLowerLayer*", function() {
    ...
    it("has as many *path* nodes as shapes in the lower layer", function() {
      expect(countShapePathsInLowerLayer()).toEqual(countShapesInLowerLayer());
    });
    ...
    it("should be able to get relayout", function(done) {
      Plotly.relayout(gd, { height: 200, width: 400 })
        .then(function() {
          expect(countShapeLowerLayerNodes()).toEqual(1);
          expect(countShapePathsInLowerLayer()).toEqual(
            countShapesInLowerLayer()
          );
        })
        .then(done);
    });
  ...
  describe("*shapeUpperLayer*", function() {
    ...
    it("has as many *path* nodes as shapes in the upper layer", function() {
      expect(countShapePathsInUpperLayer()).toEqual(countShapesInUpperLayer());
    });
    ...
    it("should be able to get relayout", function(done) {
      Plotly.relayout(gd, { height: 200, width: 400 })
        .then(function() {
          expect(countShapeUpperLayerNodes()).toEqual(1);
          expect(countShapePathsInUpperLayer()).toEqual(
            countShapesInUpperLayer()
          );
        })
        .then(done);
    });
  ...
  describe("each *subplot*", function() {
    ...
    it("has as many *path* nodes as shapes in the subplot", function() {
      expect(countShapePathsInSubplots()).toEqual(countShapesInSubplots());
    });
...
describe("Test shapes", function() {
  ...
  var testCases = [
    ...
    // xaxis.type: 'date', yaxis.type: 'category'
    {
      ...
      xaxis: { type: "date", range: ["2000-01-01", "2000-02-02"] },
  ...
  function setupLayout(testCase) {
    ...
    var x0y0 = x0 + "," + y0, x1y1 = x1 + "," + y1, x1y0 = x1 + "," + y0;
  ...
  function testDragEachShape(done) {
    ...
    expect(layoutShapes.length).toBe(4);

    // line, rect, circle and path
  ...
  function testResizeEachShape(direction, done) {
    ...
    layoutShapes.forEach(function(layoutShape, index) {
      ...
      var dx = dxToShrinkWidth[direction], dy = dyToShrinkHeight[direction];
  ...
  function testShapeResize(direction, dx, dy, layoutShape, node) {
    ...
    return resize(direction, node, dx, dy).then(function() {
      ...
      if (~direction.indexOf("n")) {
        expect(finalCoordinates[keyN] - initialCoordinates[keyN]).toBeCloseTo(
          dy
        );
      } else if (~direction.indexOf("s")) {
        expect(finalCoordinates[keyS] - initialCoordinates[keyS]).toBeCloseTo(
          dy
        );
      }

      if (~direction.indexOf("w")) {
        expect(finalCoordinates[keyW] - initialCoordinates[keyW]).toBeCloseTo(
          dx
        );
      } else if (~direction.indexOf("e")) {
        expect(finalCoordinates[keyE] - initialCoordinates[keyE]).toBeCloseTo(
          dx
        );
      }
    });
...
function waitForDragCover() {
  return new Promise(function(resolve) {
    var interval = DBLCLICKDELAY / 4, timeout = 5000;
