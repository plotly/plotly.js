...
describe('maplibre defaults', function() {
    ...
    beforeEach(function() {
        layoutOut = { font: { color: 'red' }, _subplots: {maplibre: ['maplibre']} };

        // needs a maplibre-ref in a trace in order to be detected
        fullData = [{ type: 'scattermaplibre', subplot: 'maplibre' }];
    });
    ...
    it('should fill empty containers', function() {
        ...
        expect(layoutIn).toEqual({ maplibre: {} });
    });

    it('should copy ref to input container in full (for updating on map move)', function() {
        var maplibre = { style: 'light '};

        layoutIn = { maplibre: maplibre };

        supplyLayoutDefaults(layoutIn, layoutOut, fullData);
        expect(layoutOut.maplibre._input).toBe(maplibre);
    });
    ...
    it('should accept both string and object style', function() {
        ...
        layoutIn = {
            maplibre: { style: 'light' },
            maplibre2: { style: maplibreStyleJSON }
        };

        fullData.push({ type: 'scattermaplibre', subplot: 'maplibre2' });
        layoutOut._subplots.maplibre.push('maplibre2');

        supplyLayoutDefaults(layoutIn, layoutOut, fullData);
        expect(layoutOut.maplibre.style).toEqual('light');
        expect(layoutOut.maplibre2.style).toBe(maplibreStyleJSON);
    });
    ...
    it('should fill layer containers', function() {
        ...
        supplyLayoutDefaults(layoutIn, layoutOut, fullData);
        expect(layoutOut.maplibre.layers[0].sourcetype).toEqual('geojson');
        expect(layoutOut.maplibre.layers[1].sourcetype).toEqual('geojson');
    });
    ...
    it('should skip over non-object layer containers', function() {
        ...
        supplyLayoutDefaults(layoutIn, layoutOut, fullData);
        expect(layoutOut.maplibre.layers).toEqual([jasmine.objectContaining({
            sourcetype: 'geojson',
            _index: 0
    ...
    it('should coerce \'sourcelayer\' only for *vector* \'sourcetype\'', function() {
        ...
        supplyLayoutDefaults(layoutIn, layoutOut, fullData);
        expect(layoutOut.maplibre.layers[0].sourcelayer).toEqual('layer0');
        expect(layoutOut.maplibre.layers[1].sourcelayer).toBeUndefined();
    });
    ...
    it('should only coerce relevant layer style attributes', function() {
        ...
        expect(layoutOut.maplibre.layers[0].color).toEqual('red');
        expect(layoutOut.maplibre.layers[0].line.width).toEqual(3);
        expect(layoutOut.maplibre.layers[0].fill).toBeUndefined();
        expect(layoutOut.maplibre.layers[0].circle).toBeUndefined();
        expect(layoutOut.maplibre.layers[0].symbol).toBeUndefined();

        expect(layoutOut.maplibre.layers[1].color).toEqual('blue');
        expect(layoutOut.maplibre.layers[1].fill.outlinecolor).toEqual('#d3d3d3');
        expect(layoutOut.maplibre.layers[1].line).toBeUndefined();
        expect(layoutOut.maplibre.layers[1].circle).toBeUndefined();
        expect(layoutOut.maplibre.layers[1].symbol).toBeUndefined();

        expect(layoutOut.maplibre.layers[2].color).toEqual('green');
        expect(layoutOut.maplibre.layers[2].circle.radius).toEqual(20);
        expect(layoutOut.maplibre.layers[2].line).toBeUndefined();
        expect(layoutOut.maplibre.layers[2].fill).toBeUndefined();
        expect(layoutOut.maplibre.layers[2].symbol).toBeUndefined();

        expect(layoutOut.maplibre.layers[3].color).toEqual('yellow');
        expect(layoutOut.maplibre.layers[3].symbol.icon).toEqual('monument');
        expect(layoutOut.maplibre.layers[3].line).toBeUndefined();
        expect(layoutOut.maplibre.layers[3].fill).toBeUndefined();
        expect(layoutOut.maplibre.layers[3].circle).toBeUndefined();
    });
    ...
    it('should not allow to set layer type other than *raster* for sourcetype value *raster* and *image*', function() {
        ...
        expect(layoutOut.maplibre.layers[0].type).toBe('raster');
        expect(layoutOut.maplibre.layers[1].type).toBe('raster');
    });
...
describe('maplibre plots', function() {
    var mock = require('../../image/mocks/maplibre_0.json');
    var gd;
    ...
    it('@gl should be able to toggle trace visibility', function(done) {
        ...
        Plotly.restyle(gd, 'visible', false).then(function() {
            expect(gd._fullLayout.maplibre === undefined).toBe(false);
    ...
    it('@gl should not update center while dragging', function(done) {
        ...
        _mouseEvent('mousemove', pointPos, noop).then(function() {
            ...
            return Plotly.relayout(gd, {'maplibre.center': {lat: 0, lon: 0}});
        }).then(function() {
    ...
    it('@gl should not update zoom while scroll wheeling', function(done) {
        ...
        _mouseEvent('mousemove', pointPos, noop).then(function() {
            return Plotly.relayout(gd, {'maplibre.zoom': 5});
        }).then(function() {
            ...
            return Plotly.relayout(gd, {'maplibre.zoom': 2}).then(function() {
    ...
    it('@gl should be able to relayout', function(done) {
        ...
        Plotly.relayout(gd, 'maplibre.center', { lon: 0, lat: 0 })
        .then(function() {
            ...
            return Plotly.relayout(gd, 'maplibre.zoom', '6');
        })
        .then(function() {
            expect(restyleCnt).toEqual(0);
            ...
            return Plotly.relayout(gd, 'maplibre.domain.x', [0, 0.5]);
        })
        .then(function() {
            expect(restyleCnt).toEqual(0);
            ...
            return Plotly.relayout(gd, 'maplibre.domain.y[0]', 0.5);
        })
        .then(function() {
            expect(restyleCnt).toEqual(0);
    ...
    it('@gl should be able to relayout the map style', function(done) {
        ...
        assertLayout('MapLibre Dark');

        Plotly.relayout(gd, 'maplibre.style', 'carto-positron')
        .then(function() {
            assertLayout('MapLibre Light');

            return Plotly.relayout(gd, 'maplibre.style', 'carto-darkmatter');
        })
        .then(function() {
            assertLayout('MapLibre Dark');
        })
    ...
    it('@gl should be able to add, update and remove layers', function(done) {
        var mockWithLayers = require('../../image/mocks/maplibre_layers');

        var layer0 = Lib.extendDeep({}, mockWithLayers.layout.maplibre.layers[0]);
        var layer1 = Lib.extendDeep({}, mockWithLayers.layout.maplibre.layers[1]);

        var mapUpdate = {
            'maplibre.zoom': mockWithLayers.layout.maplibre.zoom,
            'maplibre.center.lon': mockWithLayers.layout.maplibre.center.lon,
            'maplibre.center.lat': mockWithLayers.layout.maplibre.center.lat
        };

        var styleUpdate0 = {
            'maplibre.layers[0].color': 'red',
            'maplibre.layers[0].fill.outlinecolor': 'blue',
            'maplibre.layers[0].opacity': 0.3
        };

        var styleUpdate1 = {
            'maplibre.layers[1].color': 'blue',
            'maplibre.layers[1].line.width': 3,
            'maplibre.layers[1].opacity': 0.6
        };
        ...
        function getLayerLength(gd) {
            return Lib.filterVisible(gd._fullLayout.maplibre.layers || []).length;
        }
        ...
        .then(function() {
            ...
            // hide a layer
            return Plotly.relayout(gd, 'maplibre.layers[0].visible', false);
        })
        .then(function() {
            expect(getLayerLength(gd)).toEqual(1);
            expect(countVisibleLayers(gd)).toEqual(1);
            ...
            // re-show it
            return Plotly.relayout(gd, 'maplibre.layers[0].visible', true);
        })
        .then(function() {
            expect(getLayerLength(gd)).toEqual(2);
            expect(countVisibleLayers(gd)).toEqual(2);
        ...
        .then(function() {
            ...
            return Plotly.relayout(gd, 'maplibre.layers[0]', null);
        })
        .then(function() {
            expect(getLayerLength(gd)).toEqual(1);
            expect(countVisibleLayers(gd)).toEqual(1);
        ...
        .then(function() {
            ...
            return Plotly.relayout(gd, 'maplibre.layers[0]', {});
        })
        .then(function() {
            expect(gd.layout.maplibre.layers).toEqual([{}]);
            expect(countVisibleLayers(gd)).toEqual(0);

            // layer with no source are not drawn

            return Plotly.relayout(gd, 'maplibre.layers[0].source', layer0.source);
        })
        .then(function() {
            expect(getLayerLength(gd)).toEqual(1);
            expect(countVisibleLayers(gd)).toEqual(1);
    ...
    it('@gl should be able to update layer image', function(done) {
        ...
        Plotly.react(gd, makeFigure(redImage)).then(function() {
            var maplibre = gd._fullLayout.maplibre._subplot;
            map = maplibre.map;
            layerSource = map.getSource(maplibre.layerList[1].idSource);
        ...
        .then(function() {
            ...
            var maplibre = gd._fullLayout.maplibre._subplot;
            var maplibreLayers = maplibre.getMapLayers();
            var plotlyjsLayers = maplibre.layerList;

            var indexLower = maplibreLayers.findIndex(function(layer) {
                return layer.id === 'plotly-layout-layer-' + plotlyjsLayers[0].uid;
            });

            var indexUpper = maplibreLayers.findIndex(function(layer) {
    ...
    it('@gl should not wedge graph after reacting to invalid layer', function(done) {
        Plotly.react(gd, [{type: 'scattermaplibre'}], {
            maplibre: {
        ...
        .then(function() {
            return Plotly.react(gd, [{type: 'scattermaplibre'}], {
                maplibre: {
                    layers: [{
                        sourcetype: 'vector',
                        sourcelayer: 'contour',
                        source: 'maplibre://maplibre.maplibre-terrain-v2'
                    }]
                }
            });
        })
    ...
    it('@gl should not attempt to remove non-existing layer sources', function(done) {
        function _assert(msg, exp) {
            return function() {
                var layerList = gd._fullLayout.maplibre._subplot.layerList;
    ...
    it('@gl should validate layout layer input', function(done) {
        Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {
            maplibre: {
                layers: [{
                    sourcetype: 'raster',
                    source: ['']
    ...
    it('@gl should not attempt to rehover over exiting subplots', function(done) {
        ...
        .then(function() {
            return _mouseEvent('mousemove', pointPos, function() {
                ...
                expect(Fx.hover.calls.argsFor(0)[2]).toBe('maplibre');
    ...
    it('@noCI @gl should respond drag / scroll / double-click interactions', function(done) {
        ...
        function _assert(center, zoom, lon0, lat0, lon1, lat1) {
            ...
            expect([evtData['maplibre.center'].lon, evtData['maplibre.center'].lat]).toBeCloseToArray(center);
            expect(evtData['maplibre.zoom']).toBeCloseTo(zoom);
            expect(Object.keys(evtData['maplibre._derived'])).toEqual(['coordinates']);
            expect(evtData['maplibre._derived'].coordinates).toBeCloseTo2DArray([
                [lon0, lat1],
                [lon1, lat1],
                [lon1, lat0],
                [lon0, lat0]
    ...
    describe('attributions', function() {
        ...
        it('@gl should be displayed for style "Carto"', function(done) {
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {maplibre: {style: 'carto-darkmatter'}})
            .then(function() {
        ...
        ['stamen-terrain', 'stamen-toner'].forEach(function(style) {
            it('@noCI @gl should be displayed for style "' + style + '"', function(done) {
                Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {maplibre: {style: style}})
                .then(function() {
                    var s = d3SelectAll('.maplibregl-ctrl-attrib');
                    expect(s.size()).toBe(1);
        ...
        it('@noCI @gl should be displayed for style "stamen-watercolor"', function(done) {
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {maplibre: {style: 'stamen-watercolor'}})
            .then(function() {
                var s = d3SelectAll('.maplibregl-ctrl-attrib');
                expect(s.size()).toBe(1);
        ...
        it('@gl should be displayed for style "open-street-map"', function(done) {
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {maplibre: {style: 'open-street-map'}})
            .then(function() {
                var s = d3SelectAll('.maplibregl-ctrl-attrib');
                expect(s.size()).toBe(1);
        ...
        it('@gl should be displayed for style from MapLibre', function(done) {
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], {maplibre: {style: 'basic'}})
            .then(function() {
                var s = d3SelectAll('.maplibregl-ctrl-attrib');
                expect(s.size()).toBe(1);
        ...
        it('@gl should not be displayed for custom style without attribution', function(done) {
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], mockLayoutCustomStyle())
            .then(function() {
                var s = d3SelectAll('.maplibregl-ctrl-attrib');
                expect(s.size()).toBe(1);
        ...
        it('@gl should be displayed for custom style with attribution', function(done) {
            ...
            layout.maplibre.style.sources['simple-tiles'].attribution = attr;
            Plotly.newPlot(gd, [{type: 'scattermaplibre'}], layout)
            .then(function() {
                var s = d3SelectAll('.maplibregl-ctrl-attrib');
                expect(s.size()).toBe(1);
        ...
        it('@gl should be displayed for attributions defined in layers\' sourceattribution', function(done) {
            var mock = require('../../image/mocks/maplibre_layers.json');
            var customMock = Lib.extendDeep(mock);

            var attr = 'custom attribution';
            var XSS = '<img src=x onerror=\"alert(XSS);\">';
            customMock.data.pop();
            customMock.layout.maplibre.layers[0].sourceattribution = XSS + attr;
    ...
    function countVisibleTraces(gd, modes) {
        ...
        // 'modes' are the ScatterMapLibre layers names
...
describe('maplibre react', function() {
    var gd;
    ...
    it('@gl should be able to react to new tiles', function(done) {
        ...
        .then(function() {
            ...
            // new figure
            newFig.layout.maplibre.style.sources = {
                REF: {
                    type: 'raster',
                    tileSize: 256,
                    tiles: [secondLink]
...
describe('test maplibre trace/layout *below* interactions', function() {
    var gd;
    ...
    it('@gl should be able to update *below* - scattermaplibre + layout layer case', function(done) {
        function _assert(msg, exp) {
            ...
            var layoutLayerId = ['plotly-layout-layer', gd._fullLayout._uid, 'maplibre-0'].join('-');
        ...
        .then(function() { return Plotly.relayout(gd, 'maplibre.layers[0].below', 'traces'); })
        .then(function() {
        ...
        .then(function() { return Plotly.relayout(gd, 'maplibre.layers[0].below', null); })
        .then(function() {
        ...
        .then(function() { return Plotly.relayout(gd, 'maplibre.layers[0].below', 'water'); })
        .then(function() {
        ...
        .then(function() { return Plotly.relayout(gd, 'maplibre.layers[0].below', ''); })
        .then(function() {
        ...
        .then(function() { return Plotly.update(gd, {below: null}, {'maplibre.layers[0].below': null}); })
        .then(function() {
    ...
    it('@gl should be able to update *below* - scattermaplibre + choroplethmaplibre + densitymaplibre case', function(done) {
        function _assert(msg, exp) {
            var layersIds = getLayerIds();
        ...
        Plotly.newPlot(gd, [{
            ...
            type: 'densitymaplibre',
            lon: [10, 20, 30],
            lat: [15, 25, 35],
            z: [1, 20, 5],
            uid: 'density'
        }, {
            type: 'choroplethmaplibre',
            geojson: 'https://raw.githubusercontent.com/python-visualization/folium/master/examples/data/us-states.json',
            locations: ['AL'],
            z: [10],
            uid: 'choropleth'
        }], {
            maplibre: {style: 'basic'}
        })
        .then(function() {
            _assert('base', {
                scatter: [23, 24, 25, 26],
    ...
    it('@gl should be warn when *below* value does not correspond to a layer on the map', function(done) {
        ...
        .then(function() {
            // 7 for 4 scattermaplibre + 2 choroplethmaplibre + 1 densitymaplibre layer
            _assertFallback('not-gonna-work for traces', {warnCnt: 7});
        })
        .then(function() { return Plotly.relayout(gd, 'maplibre.layers[0].below', 'not-gonna-work'); })
        .then(function() {
...
describe('Test maplibre GeoJSON fetching:', function() {
    var gd;
...
describe('maplibre toImage', function() {
