...
describe("Scatter3D defaults", function() {
  ...
  var base = { x: [1, 2, 3], y: [1, 2, 3], z: [1, 2, 1] };

  it(
    "should make marker.color inherit from line.color (scalar case)",
    function() {
      var out = _supply(Lib.extendFlat({}, base, { line: { color: "red" } }));

      expect(out.line.color).toEqual("red");
      expect(out.marker.color).toEqual("red");
      expect(out.marker.line.color).toBe(
        Color.defaultLine,
        "but not marker.line.color"
      );
    }
  );

  it(
    "should make marker.color inherit from line.color (array case)",
    function() {
      var color = [1, 2, 3];

      var out = _supply(Lib.extendFlat({}, base, { line: { color: color } }));
  ...
  it("should make line.color inherit from marker.color if scalar)", function() {
    var out = _supply(Lib.extendFlat({}, base, { marker: { color: "red" } }));
  ...
  it(
    "should not make line.color inherit from marker.color if array",
    function() {
      var color = [1, 2, 3];

      var out = _supply(Lib.extendFlat({}, base, { marker: { color: color } }));
