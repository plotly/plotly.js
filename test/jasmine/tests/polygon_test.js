...
describe("polygon.tester", function() {
  ...
  it("should prepare a polygon object correctly", function() {
    ...
    polyPts.forEach(function(polyPt) {
      var poly = polygonTester(polyPt),
        xArray = polyPt.map(function(pt) {
          return pt[0];
        }),
        yArray = polyPt.map(function(pt) {
          return pt[1];
        });
  ...
  it(
    "should include the whole boundary, except as per omitFirstEdge",
    function() {
      var polyPts = [
        squareCW,
        squareCCW,
      ...
      // number of intermediate points on each edge to test
      polyPts.forEach(function(polyPt) {
        var poly = polygonTester(polyPt);
        ...
        poly.pts.forEach(function(pt1, i) {
          ...
          for (j = 1; j < np; j++) {
            if (pt0[0] === pt1[0]) {
              ...
              testPts.push([
                x,
                pt0[1] + (x - pt0[0]) * (pt1[1] - pt0[1]) / (pt1[0] - pt0[0])
              ]);
            }
          }
          testPts.forEach(function(pt, j) {
            expect(poly.contains(pt)).toBe(
              true,
              "poly: " + polyPt.join(";") + ", pt: " + pt
            );
            var isFirstEdge = i === 1 ||
              i === 2 && j === 0 ||
              i === poly.pts.length - 1 && j === 1;
            expect(poly.contains(pt, true)).toBe(
              !isFirstEdge,
              "omit: " +
                !isFirstEdge +
                ", poly: " +
  ...
  it("should find only the right interior points", function() {
    ...
    var donutpoly = polygonTester(donut), donut2poly = polygonTester(donut2);
