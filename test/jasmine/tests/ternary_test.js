...
describe("ternary plots", function() {
  ...
  describe("with scatterternary trace(s)", function() {
    ...
    it("should display to hover labels", function() {
      ...
      expect(rows[0][0].innerHTML).toEqual(
        "Component A: 0.5",
        "with correct text"
      );
      expect(rows[0][1].innerHTML).toEqual("B: 0.25", "with correct text");
      expect(rows[0][2].innerHTML).toEqual(
        "Component C: 0.25",
        "with correct text"
      );
    });
    ...
    it("should respond to hover interactions by", function() {
      ...
      expect(hoverData.curveNumber).toEqual(
        0,
        "returning the correct curve number"
      );
      expect(hoverData.pointNumber).toEqual(
        0,
        "returning the correct point number"
      );
      ...
      expect(unhoverData.curveNumber).toEqual(
        0,
        "returning the correct curve number"
      );
      expect(unhoverData.pointNumber).toEqual(
        0,
        "returning the correct point number"
      );
    ...
    it("should respond to click interactions by", function() {
      ...
      expect(ptData.curveNumber).toEqual(
        0,
        "returning the correct curve number"
      );
      expect(ptData.pointNumber).toEqual(
        0,
        "returning the correct point number"
      );
    });
  ...
  function assertRange(gd, expected) {
    ...
    var actual = [ternary.aaxis.min, ternary.baxis.min, ternary.caxis.min];
...
describe("ternary defaults", function() {
  ...
  it(
    "should coerce 'min' values to 0 and delete them for user data if they contradict",
    function() {
      layoutIn = {
        ternary: {
          aaxis: { min: 1 },
