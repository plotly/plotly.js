...
describe("Test plot api", function() {
  ...
  describe("Plotly.relayout", function() {
    ...
    it("should update the plot clipPath if the plot is resized", function(
      done
    ) {
  ...
  describe("Plotly.restyle", function() {
    ...
    it(
      "calls Scatter.arraysToCalcdata and Plots.style on scatter styling",
      function() {
        var gd = { data: [{ x: [1, 2, 3], y: [1, 2, 3] }], layout: {} };
    ...
    it(
      "can target specific traces by leaving properties undefined",
      function() {
        var gd = {
          data: [
        ...
        mockDefaultsAndCalc(gd);
        var colorDflt = [
          gd._fullData[0].marker.color,
          gd._fullData[1].marker.color
        ];
  ...
  describe("Plotly.restyle unmocked", function() {
    ...
    it("should redo auto z/contour when editing z array", function() {
      Plotly.plot(gd, [{ type: "contour", z: [[1, 2], [3, 4]] }])
        .then(function() {
          ...
          expect(gd.data[0].contours).toEqual({
            start: 1.5,
            end: 3.5,
            size: 0.5
          });
  ...
  describe("Plotly.addTraces", function() {
    ...
    it(
      "should throw an error when traces is not an object or an array of objects",
      function() {
    ...
    it("should work when newIndices is defined", function() {
      ...
      expect(PlotlyInternal.moveTraces).toHaveBeenCalledWith(gd, [-2, -1], [
        1,
        3
      ]);
    });
  ...
  describe("Plotly.moveTraces should", function() {
    ...
    it("thow an error when a current index is out of bounds", function() {
      expect(function() {
        ...
        "currentIndices must be valid indices for gd.data."
      ));
      expect(function() {
  ...
  describe("Plotly.ExtendTraces", function() {
    ...
    it(
      "should throw an error when a current index is out of bounds",
      function() {
        expect(function() {
    ...
    it(
      "should not throw an error when negative index wraps to positive",
      function() {
        expect(function() {
    ...
    it(
      "should throw an error when number of Indices does not match Update arrays",
      function() {
        expect(function() {
          Plotly.extendTraces(gd, { x: [[1, 2], [2, 3]] }, [0]);
        }).toThrow(new Error(
          "attribute x must be an array of length equal to indices array length"
        ));
    ...
    it(
      "should throw an error when maxPoints is an Object but does not match Update",
      function() {
        expect(function() {
    ...
    it(
      "should throw an error when update keys mismatch trace keys",
      function() {
    ...
    it(
      "should extend and window traces using full maxPoint object",
      function() {
  ...
  describe("Plotly.redraw", function() {
    ...
    it("", function(done) {
      var gd = createGraphDiv(), initialData = [], layout = { title: "Redraw" };
  ...
  describe("cleanData & cleanLayout", function() {
    ...
    it("should rename 'YIGnBu' colorscales YlGnBu (2dMap case)", function() {
      var data = [{ type: "heatmap", colorscale: "YIGnBu" }];
    ...
    it(
      "should rename 'YIGnBu' colorscales YlGnBu (markerColorscale case)",
      function() {
        var data = [{ type: "scattergeo", marker: { colorscale: "YIGnBu" } }];
    ...
    it("should rename 'YIOrRd' colorscales YlOrRd (2dMap case)", function() {
      var data = [{ type: "contour", colorscale: "YIOrRd" }];
    ...
    it(
      "should rename 'YIOrRd' colorscales YlOrRd (markerColorscale case)",
      function() {
        var data = [{ type: "scattergeo", marker: { colorscale: "YIOrRd" } }];
    ...
    it("should rename 'highlightColor' to 'highlightcolor')", function() {
      var data = [
        ...
        { type: "surface" },
        { type: "surface", contours: false },
        { type: "surface", contours: { stuff: {}, x: false, y: [] } }
      ];
    ...
    it(
      "should rename *filtersrc* to *target* in filter transforms",
      function() {
        var data = [
          {
            transforms: [
              { type: "filter", filtersrc: "y" },
              { type: "filter", operation: "<" }
            ]
          },
          { transforms: [{ type: "filter", target: "y" }] }
        ];

        Plotly.plot(gd, data);

        var trace0 = gd.data[0], trace1 = gd.data[1];
    ...
    it(
      "should rename *calendar* to *valuecalendar* in filter transforms",
      function() {
        var data = [
          {
            transforms: [
              { type: "filter", target: "y", calendar: "hebrew" },
              { type: "filter", operation: "<" }
            ]
          },
          { transforms: [{ type: "filter", valuecalendar: "jalali" }] }
        ];
    ...
    it("should cleanup annotations / shapes refs", function() {
      ...
      var layout = {
        annotations: [{ ref: "paper" }, null, { xref: "x02", yref: "y1" }],
        shapes: [{ xref: "y", yref: "x" }, null, { xref: "x03", yref: "y1" }]
      };

      Plotly.plot(gd, data, layout);

      expect(gd.layout.annotations[0]).toEqual({
        xref: "paper",
        yref: "paper"
      });
  ...
  describe("Plotly.newPlot", function() {
    ...
    it("should respect layout.width and layout.height", function(done) {
      ...
      var data = [{ x: [1, 2], y: [1, 2] }];

      Plotly.plot(gd, data).then(function() {
        var height = 50;

        Plotly.newPlot(gd, data, { height: height })
          .then(function() {
  ...
  describe("Plotly.update should", function() {
    ...
    it(
      "clear calcdata on data + axis updates w/o extending current gd.data",
      function(done) {
        var traceUpdate = { x: [[3, 1, 3]] };

        var layoutUpdate = { xaxis: { title: "A", type: "-" } };
