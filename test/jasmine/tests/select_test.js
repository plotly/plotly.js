...
describe("select box and lasso", function() {
  ...
  describe("select elements", function() {
    ...
    it("should be appended to the zoom layer", function(done) {
      var x0 = 100, y0 = 200, x1 = 150, y1 = 250, x2 = 50, y2 = 50;

      gd.once("plotly_selecting", function() {
        expect(d3.selectAll(".zoomlayer > .zoombox-corners").size()).toEqual(1);
        expect(d3.selectAll(".zoomlayer > .select-outline").size()).toEqual(2);
      });
      ...
      gd.once("plotly_deselect", function() {
        expect(d3.selectAll(".zoomlayer > .select-outline").size()).toEqual(0);
      });
  ...
  describe("select events", function() {
    ...
    it("should trigger selecting/selected/deselect events", function(done) {
      var selectingCnt = 0, selectingData;
      gd.on("plotly_selecting", function(data) {
        selectingCnt++;
        selectingData = data;
      });

      var selectedCnt = 0, selectedData;
      ...
      expect(selectingData.points).toEqual(
        [
          { curveNumber: 0, pointNumber: 0, x: 0.002, y: 16.25, id: undefined },
          { curveNumber: 0, pointNumber: 1, x: 0.004, y: 12.5, id: undefined }
        ],
  ...
  describe("lasso events", function() {
    ...
    it("should trigger selecting/selected/deselect events", function(done) {
      ...
      expect(selectingData.points).toEqual(
        [{ curveNumber: 0, pointNumber: 10, x: 0.099, y: 2.75, id: undefined }],
        "with the correct selecting points"
      );
