...
describe("Test hover and click interactions", function() {
  ...
  var mock3 = {
    ...
    layout: {
      xaxis: { range: [0, 4], autorange: true },
      yaxis: { range: [0, 4], autorange: true },
      height: 450,
      width: 1000,
      autosize: true
    }
  ...
  describe("hover event is fired on hover", function() {
    ...
    it("in general", function(done) {
      ...
      Plotly.plot(
        gd,
        modifiedMockCopy.data,
        modifiedMockCopy.layout
    ...
    it("even when hoverinfo (== plotly tooltip) is set to none", function(
      done
    ) {
      var modifiedMockCopy = Lib.extendDeep({}, mockCopy);
      ...
      Plotly.plot(
        ...
        window.setTimeout(
          function() {
            var x = 654, y = 316;
            // we start here
            var canceler = window.setInterval(
              function() {
