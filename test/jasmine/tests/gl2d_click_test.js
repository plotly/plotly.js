...
Plotly.register([require('@lib/contourgl')]);
...
describe('Test hover and click interactions', function() {
  ...
  it('should output correct event data for scattergl', function(done) {
    ...
    Plotly.plot(gd, _mock).then(run).catch(fail).then(done);
  });

  it("should output correct event data for scattergl with hoverinfo: 'none'", function(
    done
  ) {
    var _mock = Lib.extendDeep({}, mock1);
  ...
  it('should output correct event data for scattergl after visibility restyle', function(
    done
  ) {
