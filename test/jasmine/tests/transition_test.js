...
function runTests(transitionDuration) {
  describe(
    "Plots.transition (duration = " + transitionDuration + ")",
    function() {
      "use strict";
      var gd;
      ...
      it("transitions an annotation", function(done) {
        function annotationPosition() {
          var g = gd._fullLayout._infolayer
            .select(".annotation")
        ...
        Plotly.relayout(gd, { annotations: [{ x: 0, y: 0, text: "test" }] })
          .then(function() {
            ...
            return Plots.transition(
              gd,
              null,
              { "annotations[0].x": 1, "annotations[0].y": 1 },
              [],
