...
describe('Test click interactions:', function() {
  ...
  var pointPos = [344, 216], blankPos = [63, 356];
  ...
  describe('click event with hoverinfo set to skip - plotly_click', function() {
    ...
    beforeEach(function(done) {
      ...
      Plotly.plot(gd, modifiedMockCopy.data, modifiedMockCopy.layout).then(
        done
      );
  ...
  describe('plotly_unhover event with hoverinfo set to none', function() {
    ...
    it('should contain the correct fields despite hoverinfo: "none"', function(
      ...
      move(pointPos[0], pointPos[1], blankPos[0], blankPos[1])
        .then(function() {
          expect(futureData.points.length).toEqual(1);
  ...
  describe('drag interactions', function() {
    ...
    it('on nw dragbox should update the axis ranges', function(done) {
      ...
      drag(pos[0], pos[1], pos[0] + 10, pos[1] + 50)
        .then(function() {
    ...
    it('on ne dragbox should update the axis ranges', function(done) {
      ...
      drag(pos[0], pos[1], pos[0] + 50, pos[1] + 50)
        .then(function() {
          expect(gd.layout.xaxis.range).toBeCloseToArray([
            -3.01196749,
            1.72466470,
  ...
  describe('double click interactions', function() {
    var setRangeX = [-3, 1], setRangeY = [-0.5, 1];

    var zoomRangeX = [-2, 0], zoomRangeY = [0, 0.5];
    ...
    it("when set to 'reset+autorange' (the default) should work when 'autorange' is on", function(
      done
    ) {
    ...
    it("when set to 'reset+autorange' (the default) should reset to set range on double click", function(
      done
    ) {
    ...
    it("when set to 'reset+autorange' (the default) should autosize on 1st double click and reset on 2nd", function(
      done
    ) {
      mockCopy = setRanges(mockCopy);
    ...
    it("when set to 'reset+autorange' (the default) should autosize on 1st double click and zoom when immediately dragged", function(
      done
    ) {
      mockCopy = setRanges(mockCopy);
    ...
    it("when set to 'reset+autorange' (the default) should follow updated auto ranges", function(
      done
    ) {
      var updateData = {
        x: [[1e-4, 0, 1e3]],
    ...
    it("when set to 'reset' should work when 'autorange' is on", function(
      done
    ) {
      Plotly.plot(gd, mockCopy.data, mockCopy.layout, { doubleClick: 'reset' })
        .then(function() {
          expect(gd.layout.xaxis.range).toBeCloseToArray(autoRangeX);
          expect(gd.layout.yaxis.range).toBeCloseToArray(autoRangeY);
    ...
    it("when set to 'reset' should reset to set range on double click", function(
      done
    ) {
      mockCopy = setRanges(mockCopy);
    ...
    it("when set to 'autosize' should work when 'autorange' is on", function(
      done
    ) {
      Plotly.plot(gd, mockCopy.data, mockCopy.layout, {
    ...
    it("when set to 'autosize' should set to autorange on double click", function(
      done
    ) {
      mockCopy = setRanges(mockCopy);
    ...
    it("when set to 'autosize' should reset on all double clicks", function(
      done
    ) {
      mockCopy = setRanges(mockCopy);
  ...
  describe('pan interactions', function() {
    ...
    it('should move the plot when panning', function() {
      var start = 100, end = 300, plot = gd._fullLayout._plots.xy.plot;
