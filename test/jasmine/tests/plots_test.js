...
describe("Test Plots", function() {
  ...
  describe("Plots.supplyDefaults", function() {
    it("should not throw an error when gd is a plain object", function() {
      var height = 100, gd = { layout: { height: height } };
    ...
    it("should relink private keys", function() {
      var oldFullData = [
        { type: "scatter3d", z: [1, 2, 3] },
        { type: "contour", _empties: [1, 2, 3] }
      ];
      ...
      var newData = [
        { type: "scatter3d", z: [1, 2, 3, 4] },
        { type: "contour", z: [[1, 2, 3], [2, 3, 4]] }
      ];

      var newLayout = { annotations: [{}, {}, {}] };
      ...
      expect(gd._fullLayout.annotations[0]._min).toBe(
        oldFullLayout.annotations[0]._min
      );
      expect(gd._fullLayout.annotations[1]._max).toBe(
        oldFullLayout.annotations[1]._max
      );
      expect(gd._fullLayout.someFunc).toBe(oldFullLayout.someFunc);

      expect(gd._fullLayout.xaxis.c2p).not.toBe(
        oldFullLayout.xaxis.c2p,
        "(set during ax.setScale"
      );
      expect(gd._fullLayout.yaxis._m).not.toBe(
        oldFullLayout.yaxis._m,
        "(set during ax.setScale"
      );
    });
    ...
    it(
      "should call sanitizeMargins only once when both width and height are defined",
      function() {
        var gd = { layout: { width: 100, height: 100 } };
    ...
    it(
      "should call sanitizeMargins only once when autosize is false",
      function() {
        var gd = { layout: { autosize: false, height: 100 } };
    ...
    it(
      "should call sanitizeMargins only once when autosize is true",
      function() {
        var gd = { layout: { autosize: true, height: 100 } };
  ...
  describe("Plots.supplyLayoutGlobalDefaults should", function() {
    var layoutIn, layoutOut, expected;
    ...
    it("should sanitize margins when they are wider than the plot", function() {
      layoutIn = { width: 500, height: 500, margin: { l: 400, r: 200 } };
      expected = { l: 332, r: 166, t: 100, b: 80, pad: 0, autoexpand: true };
    ...
    it(
      "should sanitize margins when they are taller than the plot",
      function() {
        layoutIn = {
          width: 500,
          height: 500,
          margin: { l: 400, r: 200, t: 300, b: 500 }
        };
        expected = { l: 332, r: 166, t: 187, b: 311, pad: 0, autoexpand: true };
  ...
  describe("Plots.supplyTraceDefaults", function() {
    var supplyTraceDefaults = Plots.supplyTraceDefaults, layout = {};
  ...
  describe("Plots.getSubplotIds", function() {
    ...
    it("returns scene ids in order", function() {
      var layout = { scene2: {}, scene: {}, scene3: {} };

      expect(getSubplotIds(layout, "gl3d")).toEqual([
        "scene",
        "scene2",
        "scene3"
      ]);

      expect(getSubplotIds(layout, "cartesian")).toEqual([]);
      expect(getSubplotIds(layout, "geo")).toEqual([]);
      expect(getSubplotIds(layout, "no-valid-subplot-type")).toEqual([]);
    });

    it("returns geo ids in order", function() {
      var layout = { geo2: {}, geo: {}, geo3: {} };

      expect(getSubplotIds(layout, "geo")).toEqual(["geo", "geo2", "geo3"]);
    ...
    it("returns cartesian ids", function() {
      var layout = { _has: Plots._hasPlotType, _plots: { xy: {}, x2y2: {} } };

      expect(getSubplotIds(layout, "cartesian")).toEqual([]);

      layout._basePlotModules = [{ name: "cartesian" }];
      expect(getSubplotIds(layout, "cartesian")).toEqual(["xy", "x2y2"]);
      expect(getSubplotIds(layout, "gl2d")).toEqual([]);

      layout._basePlotModules = [{ name: "gl2d" }];
      expect(getSubplotIds(layout, "gl2d")).toEqual(["xy", "x2y2"]);
      expect(getSubplotIds(layout, "cartesian")).toEqual([]);
    });
  });
  ...
  describe("Plots.resize", function() {
    ...
    it("should update the axis scales", function() {
      var fullLayout = gd._fullLayout, plotinfo = fullLayout._plots.xy;
  ...
  describe("extendObjectWithContainers", function() {
    ...
    it("extend each container items", function() {
      var dest = {
        container: [{ text: "1", x: 1, y: 1 }, { text: "2", x: 2, y: 2 }]
      };

      var src = { container: [{ text: "1-new" }, { text: "2-new" }] };
    ...
    it("clears container items when applying null src items", function() {
      ...
      var src = { container: [null, null] };

      var expected = { container: [null, null] };
    ...
    it("should serialize data, layout and frames", function(done) {
      var mock = {
        ...
        frames: [
          {
            data: [{ y: [1, 2, 1] }],
            layout: { title: "frame A" },
            name: "A"
          },
          null,
          {
            data: [{ y: [1, 2, 3] }],
            layout: { title: "frame B" },
            name: "B"
          },
          {
      ...
      Plotly.plot(createGraphDiv(), mock)
        .then(function(gd) {
  ...
  describe("Plots.getSubplotCalcData", function() {
    ...
    var cd = [[{ trace: trace0 }], [{ trace: trace1 }], [{ trace: trace2 }]];

    it(
      "should extract calcdata traces associated with subplot (1)",
      function() {
    ...
    it(
      "should extract calcdata traces associated with subplot (2)",
      function() {
  ...
  describe("Plots.generalUpdatePerTraceModule", function() {
    ...
    it(
      "should update subplot trace hash and call module plot method with correct calcdata traces",
      function() {
        var out = _update([
    ...
    it("should handle cases when module plot is not set (geo case)", function(
      done
    ) {
    ...
    it(
      "should handle cases when module plot is not set (ternary case)",
      function(done) {
        Plotly.plot(createGraphDiv(), [
          {
            type: "scatterternary",
          ...
          { type: "scatterternary", a: [0.1, 0.2], b: [0.2, 0.1] }
        ]).then(function() {
