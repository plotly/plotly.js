...
describe("Plotly.Snapshot", function() {
  ...
  describe("clone", function() {
    var data, layout, dummyTrace1, dummyTrace2, dummyGraphObj;
    ...
    dummyGraphObj = { data: data, layout: layout };

    it(
      "should create a themeTile, with width certain things stripped out",
      function() {
        var themeOptions = { tileClass: "themes__thumb" };
        ...
        expect(themeTile.layout.height).toEqual(
          THEMETILE_DEFAULT_LAYOUT.height
        );
        ...
        expect(themeTile.gd).toBe(themeTile.td);
    ...
    it(
      "should create a thumbnail for image export to the filewell",
      function() {
        var thumbnailOptions = { tileClass: "thumbnail" };
        ...
        expect(thumbTile.layout.hidesources).toEqual(
          THUMBNAIL_DEFAULT_LAYOUT.hidesources
        );
        expect(thumbTile.layout.showlegend).toEqual(
          THUMBNAIL_DEFAULT_LAYOUT.showlegend
        );
        expect(thumbTile.layout.borderwidth).toEqual(
          THUMBNAIL_DEFAULT_LAYOUT.borderwidth
        );
        expect(thumbTile.layout.annotations).toEqual(
          THUMBNAIL_DEFAULT_LAYOUT.annotations
        );
      }
    );
    ...
    it("should create a 3D thumbnail with limited attributes", function() {
      var figure = {
        ...
        layout: { autosize: true, scene: { aspectratio: { y: 1, x: 1, z: 1 } } }
      };
    ...
    it(
      "should not touch the data or layout if you do not specify an existing tileClass",
      function() {
        var vanillaOptions = { tileClass: "notarealclass" };

        var vanillaPlotTile = Plotly.Snapshot.clone(
          dummyGraphObj,
          vanillaOptions
        );
    ...
    it("should set the background parameter appropriately", function() {
      ...
      pt = Plotly.Snapshot.clone(dummyGraphObj, { setBackground: "blue" });
      expect(pt.config.setBackground).toEqual("blue");
    });
  });

  describe("toSVG", function() {
    var parser = new DOMParser(), gd;
    ...
    it("should not return any nested svg tags of annotations", function(done) {
      Plotly.plot(gd, annotationMock.data, annotationMock.layout)
        .then(function() {
          return Plotly.Snapshot.toSVG(gd);
        })
        .then(function(svg) {
    ...
    it(
      "should force *visibility: visible* for text elements with *visibility: inherit*",
      function(done) {
