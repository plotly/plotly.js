...
describe("filter transforms defaults:", function() {
  ...
  it("supplyTraceDefaults should coerce all attributes", function() {
    traceIn = { x: [1, 2, 3], transforms: [{ type: "filter", value: 0 }] };
  ...
  it(
    "supplyTraceDefaults should not coerce attributes if enabled: false",
    function() {
      traceIn = {
        x: [1, 2, 3],
        transforms: [{ enabled: false, type: "filter", value: 0 }]
      };

      traceOut = Plots.supplyTraceDefaults(traceIn, 0, fullLayout);

      expect(traceOut.transforms).toEqual([
        { type: "filter", enabled: false, _module: Filter }
      ]);
    }
  );

  it(
    "supplyTraceDefaults should coerce *target* as a strict / noBlank string",
    function() {
      traceIn = {
        x: [1, 2, 3],
        transforms: [
          { type: "filter" },
          { type: "filter", target: 0 },
          { type: "filter", target: "" },
          { type: "filter", target: "marker.color" }
        ]
      };
...
describe("filter transforms calc:", function() {
  ...
  function _transform(data, layout) {
    var gd = { data: data, layout: layout || {} };
  ...
  var base = {
    ...
    marker: { color: [0.1, 0.2, 0.3, 0.1, 0.2, 0.3, 0.4], size: 20 },
  ...
  it("filters should skip if *target* isn't present in trace", function() {
    var out = _transform([
      Lib.extendDeep({}, base, {
        transforms: [{ type: "filter", operation: ">", value: 0, target: "z" }]
      })
    ]);
  ...
  it("filters should handle 3D *z* data", function() {
    var out = _transform([
      Lib.extendDeep({}, base, {
        ...
        transforms: [
          { type: "filter", operation: ">", value: "2016-10-01", target: "z" }
        ]
      })
    ]);
  ...
  it(
    "should use the calendar from the target attribute if target is a string",
    function() {
  ...
  it(
    "should use targetcalendar anyway if there is no matching calendar attribute",
    function() {
      // this is the same data as in "filters should handle 3D *z* data"
      // but with different calendars
      var out = _transform([
  ...
  it("filters should handle geographical *lon* data", function() {
    var trace0 = {
      ...
      lat: [-50, -40, 10, 20, 30],
      transforms: [{ type: "filter", operation: ">", value: 0, target: "lon" }]
    };
    ...
    var trace1 = {
      ...
      lon: [-90, -40, 100, 120, 130],
      lat: [-50, -40, 10, 20, 30],
      transforms: [{ type: "filter", operation: "<", value: 0, target: "lat" }]
    };
  ...
  it("filters should handle nested attributes", function() {
    var out = _transform([
      Lib.extendDeep({}, base, {
        transforms: [
          { type: "filter", operation: ">", value: 0.2, target: "marker.color" }
        ]
      })
    ]);
  ...
  it("filters should chain as AND (case 1)", function() {
    var out = _transform([
      Lib.extendDeep({}, base, {
        transforms: [
          { type: "filter", operation: ">", value: 0, target: "x" },
          { type: "filter", operation: "<", value: 3, target: "x" }
        ]
      })
    ]);
    ...
    var out = _transform([
      Lib.extendDeep({}, base, {
        transforms: [
          ...
          { type: "filter", operation: "<", value: 2, target: "y" }
        ]
      })
    ]);
  ...
  describe("filters should handle numeric values", function() {
    ...
    function _assert(out, x, y, markerColor) {
      ...
      expect(out[0].marker.color).toEqual(
        markerColor,
        "- marker.color arrayOk"
      );
    ...
    it("with operation *[]*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "[]", value: [-1, 1], target: "x" }]
        })
      ]);

      _assert(out, [-1, 0, 1], [2, 1, 2], [0.2, 0.1, 0.2]);
    });
    ...
    it("with operation *)(*", function() {
      ...
      _assert(out, [-2, -2, 2, 3], [1, 3, 3, 1], [0.1, 0.3, 0.3, 0.4]);
    });
    ...
    it("with operation *)[*", function() {
      ...
      _assert(out, [-2, -2, 1, 2, 3], [1, 3, 2, 3, 1], [
        0.1,
        0.3,
        0.2,
        0.3,
    ...
    it("with operation *](*", function() {
      ...
      _assert(out, [-2, -1, -2, 2, 3], [1, 2, 3, 3, 1], [
        0.1,
        0.2,
        0.3,
        0.3,
    ...
    it("with operation *][*", function() {
      ...
      _assert(out, [-2, -1, -2, 1, 2, 3], [1, 2, 3, 2, 3, 1], [
        0.1,
        0.2,
        0.3,
        0.2,
    ...
    it("with operation *{}*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "{}", value: [-2, 0], target: "x" }]
        })
      ]);

      _assert(out, [-2, -2, 0], [1, 3, 1], [0.1, 0.3, 0.1]);
    });
    ...
    it("with operation *}{*", function() {
      ...
      _assert(out, [-1, 1, 2, 3], [2, 2, 3, 1], [0.2, 0.2, 0.3, 0.4]);
    });
    ...
    it("should honored set axis type", function() {
      var out = _transform(
        [
          Lib.extendDeep({}, _base, {
            x: [1, 2, 3, 0, -1, -2, -3],
            transforms: [{ operation: ">", value: -1, target: "x" }]
          })
        ],
        { xaxis: { type: "category" } }
      );
  ...
  describe("filters should handle categories", function() {
    ...
    it("with operation *()*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "()", value: ["a", "c"], target: "x" }]
        })
      ]);
    ...
    it("with operation *{}*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "{}", value: ["b", "d"], target: "x" }]
        })
      ]);
  ...
  describe("filters should handle dates", function() {
    var _base = {
      ...
      y: [1, 2, 3, 1, 5],
      marker: { line: { color: [0.1, 0.2, 0.3, 0.1, 0.2], width: 2.5 } },
      transforms: [{ type: "filter" }]
    };
    ...
    function _assert(out, x, y, markerLineColor) {
      expect(out[0].x).toEqual(x, "- x coords");
      expect(out[0].y).toEqual(y, "- y coords");
      expect(out[0].marker.line.color).toEqual(
        markerLineColor,
        "- marker.line.color arrayOk"
      );
      expect(out[0].marker.line.width).toEqual(
        2.5,
    ...
    it("with operation *=*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "=", value: ["2015-07-20"], target: "x" }]
        })
      ]);
    ...
    it("with operation *<*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: "<", value: "2016-01-01", target: "x" }]
        })
      ]);
    ...
    it("with operation *>*", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [{ operation: ">=", value: "2016-08-01", target: "x" }]
        })
      ]);
    ...
    it("with operation *)(*", function() {
      ...
      _assert(out, ["2015-07-20", "2016-10-21", "2016-12-02"], [1, 1, 5], [
        0.1,
        0.1,
        0.2
      ]);
  ...
  it("filters should handle ids", function() {
    var out = _transform([
      Lib.extendDeep({}, base, {
        transforms: [
          { operation: "{}", value: ["p1", "p2", "n1"], target: "ids" }
        ]
      })
    ]);
  ...
  describe("filters should handle array *target* values", function() {
    ...
    it("with numeric items", function() {
      var out = _transform([
        Lib.extendDeep({}, _base, {
          transforms: [
            { target: [1, 1, 0, 0, 1, 0, 1], operation: "{}", value: 0 }
          ]
        })
      ]);
...
describe("filter transforms interactions", function() {
  ...
  var mockData0 = [
    {
      ...
      y: [1, 2, 3, 1, 2, 3, 1],
      text: ["a", "b", "c", "d", "e", "f", "g"],
      transforms: [{ type: "filter", operation: ">" }]
    }
  ];
  ...
  var mockData1 = [
    Lib.extendDeep({}, mockData0[0]),
    {
      ...
      y: [1, 2, 3, 2, 5, 2, 0],
      text: ["A", "B", "C", "D", "E", "F", "G"],
      transforms: [{ type: "filter", operation: "<", value: 10 }]
    }
  ];
  ...
  it("Plotly.restyle should work", function(done) {
    ...
    function assertUid(gd) {
      expect(gd._fullData[0].uid).toEqual(
        uid + "0",
        "should preserve uid on restyle"
      );
    }
  ...
  it("Plotly.extendTraces should work", function(done) {
    ...
    Plotly.plot(gd, data)
      .then(function() {
        ...
        return Plotly.extendTraces(gd, { x: [[-3, 4, 5]], y: [[1, -2, 3]] }, [
          0
        ]);
      })
      .then(function() {
        ...
        return Plotly.addTraces(gd, [
          {
            ...
            x: ["h", "i"],
            y: [2, 1],
            transforms: [{ type: "filter", operation: "=", value: "i" }]
          }
        ]);
      })
      .then(function() {
