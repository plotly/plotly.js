...
function generate(maxJitter) {
  var x = d3.range(-1, 1.5, 0.5);
  // left closed, right open interval
  var y = d3.range(-1, 1.5, 0.5);
  // left closed, right open interval
    ...
    data: [
      {
        ...
        y: model.y,
        z: transpose(model.z),
        // gl-vis is column-major order while ploly is row-major order
        colorscale: "Jet",
        contours: { start: 0, end: 2, size: 0.1, coloring: "fill" },
        uid: "ad5624",
        zmin: 0,
        zmax: 2
      }
...
describe("contourgl plots", function() {
  ...
  it('render without raising an error (coloring: "lines")', function(done) {
    ...
    mock.data[0].contours.coloring = "lines";
    // 'fill' is the default
    makePlot(gd, mock, done);
  });

  it(
    'render smooth, regular ellipses without raising an error (coloring: "fill")',
    function(done) {
  ...
  it(
    'render ellipses with added noise without raising an error (coloring: "fill")',
    function(done) {
      var mock = plotDataElliptical(0.5);
      mock.data[0].contours.coloring = "fill";
      // 'fill' is the default
