...
describe("groupby", function() {
  describe("one-to-many transforms:", function() {
    ...
    it("Plotly.restyle should work", function(done) {
      ...
      Plotly.plot(gd, data)
        .then(function() {
          assertStyle(dims, ["rgb(255, 0, 0)", "rgb(0, 0, 255)"], [1, 1]);

          return Plotly.restyle(gd, "marker.opacity", 0.4);
        })
        .then(function() {
          assertStyle(dims, ["rgb(255, 0, 0)", "rgb(0, 0, 255)"], [0.4, 0.4]);
        ...
        .then(function() {
          assertStyle(dims, ["rgb(0, 128, 0)", "rgb(255, 0, 0)"], [0.4, 0.4]);
  ...
  describe(
    ...
    function() {
          ...
          transforms: [
            {
              ...
              style: {
                a: {
                  marker: { color: "orange", size: 20, line: { color: "red" } }
                },
                b: {
                  mode: "markers+lines",
                  // heterogeonos attributes are OK: group 'a' doesn't need to define this
                  marker: {
                    color: "cyan",
                    size: 15,
                    line: { color: "purple" },
          ...
          transforms: [
            {
              ...
              style: {
                ...
                b: {
                  marker: { size: 15, line: { color: "yellow" } },
                  line: { color: "purple" }
                }
              }
            }
      ...
      var mockData5 = [
        {
          ...
          transforms: [
            { type: "groupby", groups: ["a", "a", "b", "a", "b", "b", "a"] }
          ]
        }
      ];

      it(
        "`data` preserves user supplied input but `gd._fullData` reflects the grouping",
        test(mockData1)
      );

      it(
        "passes with lots of attributes and heterogenous attrib presence",
        test(mockData2)
      );

      it(
        "passes with group styles partially overriding top level aesthetics",
        test(mockData3)
      );
      it(
        "passes extended tests with group styles partially overriding top level aesthetics",
        function(done) {
          var data = Lib.extendDeep([], mockData3);
          var gd = createGraphDiv();
          Plotly.plot(gd, data).then(function() {
            expect(gd._fullData[0].marker.line.color).toEqual([
              "orange",
              "red",
              "cyan",
              "pink"
      ...
      it(
        "passes with no explicit styling for the individual group",
        test(mockData4)
      );

      it(
        "passes with no explicit styling in the group transform at all",
        test(mockData5)
      );
    }
  );
    ...
    function test(mockData) {
      return function(done) {
        ...
        Plotly.plot(gd, data).then(function() {
          ...
          expect(gd._fullData[0].ids).toEqual([
            "q",
            "w",
            "r",
            "t",
          ...
          expect(gd._fullData[0].marker.line.width).toEqual([
            4,
            2,
            4,
            2,
