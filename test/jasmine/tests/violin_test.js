...
describe('Test violin hover:', function () {
        ...
        return Plotly.newPlot(gd, fig).then(function () {
            ...
            if (specs.hoverLabelPos) {
                d3SelectAll('g.hovertext').each(function (_, i) {
                    ...
                    expect([bbox.bottom, bbox.top]).toBeWithinArray(
                        specs.hoverLabelPos[i],
                        10,
                        'bottom--top hover label ' + i
                    );
                });
            }
        });
    ...
    [
        ...
        },
        {
            ...
            nums: [
                '(median: 0.7, day 2)',
                '(min: 0.2, day 2)',
                '(lower fence: 0.2, day 2)',
                '(q1: 0.5, day 2)',
                '(q3: 0.8, day 2)',
                '(upper fence: 0.9, day 2)',
                '(max: 0.9, day 2)'
            ],
            name: ['radishes', '', '', '', '', '', ''],
            isRotated: true
        ...
        },
        {
            ...
            pos: [430, 130],
            nums: [
                '(upper fence: 0.9, 2018 - day 2)',
                '(lower fence: 0.1, 2018 - day 2)',
                '(max: 0.9, 2018 - day 2)',
                '(min: 0.1, 2018 - day 2)',
                '(q1: 0.2, 2018 - day 2)',
                '(q3: 0.7, 2018 - day 2)',
    ...
    describe('KDE lines inside violin under *kde* hoveron flag', function () {
        ...
        function assertViolinHoverLine(pos) {
            ...
            expect([line.attr('x1'), line.attr('y1'), line.attr('x2'), line.attr('y2')]).toBeCloseToArray(
                pos,
                'line position'
            );
        }
...
describe('Test violin restyle:', function () {
    ...
    it('should be able to add/remove innner parts', function (done) {
        ...
        function _assertOne(msg, exp, trace3, k, query) {
            expect(trace3.selectAll(query).size()).toBe(exp[k] || 0, k + ' - ' + msg);
        }
