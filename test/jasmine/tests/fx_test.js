...
describe("relayout", function() {
  ...
  it("should update main drag with correct", function(done) {
    function assertMainDrag(cursor, isActive) {
      expect(d3.selectAll("rect.nsewdrag").size()).toEqual(
        1,
        "number of nodes"
      );
      var mainDrag = d3.select("rect.nsewdrag"), node = mainDrag.node();

      expect(mainDrag.classed("cursor-" + cursor)).toBe(
        true,
        "cursor " + cursor
      );
