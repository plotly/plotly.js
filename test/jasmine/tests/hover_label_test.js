...
describe("hover info", function() {
  ...
  var mock = require("@mocks/14.json"),
    evt = { clientX: mock.layout.width / 2, clientY: mock.layout.height / 2 };
  ...
  describe("hover info text", function() {
    ...
    it("responds to hover text", function() {
      ...
      expect(d3.selectAll("g.axistext").size()).toEqual(0);
      expect(d3.selectAll("g.hovertext").size()).toEqual(1);
      expect(d3.selectAll("g.hovertext").select("text").html()).toEqual(
        "hover text  with spaces  not newlines"
      );
    });
  });
  ...
  describe("hover info all", function() {
    ...
    it("responds to hover all", function() {
      ...
      expect(d3.selectAll("g.axistext").select("text").html()).toEqual("0.388");
      expect(
        d3.selectAll("g.hovertext").select("text").selectAll("tspan").size()
      ).toEqual(2);
      expect(
        d3.selectAll("g.hovertext").selectAll("tspan")[0][0].innerHTML
      ).toEqual("1");
      expect(
        d3.selectAll("g.hovertext").selectAll("tspan")[0][1].innerHTML
      ).toEqual("hover text");
    });
  });
  ...
  describe("hover info with bad name", function() {
    ...
    it("cleans the name", function() {
      ...
      expect(
        d3.selectAll("g.hovertext").selectAll("text.name").node().innerHTML
      ).toEqual("&lt;img src=x o...");
    });
  });
  ...
  describe("hover error x text (log axis positive)", function() {
    ...
    it("responds to hover x+text", function() {
      ...
      expect(d3.selectAll("g.axistext").size()).toEqual(1);
      expect(d3.selectAll("g.axistext").select("text").html()).toEqual(
        "0.388 \xB1 1"
      );
    });
  });
  ...
  describe("hoverformat", function() {
    var data = [{ x: [1, 2, 3], y: [0.12345, 0.23456, 0.34567] }],
      layout = {
...
describe("hover info on stacked subplots", function() {
  ...
  describe("hover info on stacked subplots with shared y-axis", function() {
    ...
    it("responds to hover", function() {
      ...
      expect(gd._hoverdata[0]).toEqual(
        jasmine.objectContaining({ curveNumber: 0, pointNumber: 0, x: 1, y: 0 })
      );
      ...
      expect(gd._hoverdata[2]).toEqual(
        jasmine.objectContaining({ curveNumber: 2, pointNumber: 0, x: 3, y: 0 })
      );
...
describe("hover info on overlaid subplots", function() {
  ...
  it("should respond to hover", function(done) {
    ...
    Plotly.plot(createGraphDiv(), mock.data, mock.layout)
      .then(function() {
        ...
        expect(hoverText.size()).toEqual(
          2,
          "with 2 labels on the overlaid pts"
        );

        expect(axisText.select("text").html()).toEqual(
          "1",
          "with correct axis label"
        );

        var textNodes = hoverText.selectAll("text");

        expect(textNodes[0][0].innerHTML).toEqual(
          "Take Rate",
          "with correct hover labels"
        );
        expect(textNodes[0][1].innerHTML).toEqual(
          "0.35",
          "with correct hover labels"
        );
        expect(textNodes[1][0].innerHTML).toEqual(
          "Revenue",
          "with correct hover labels"
        );
        expect(textNodes[1][1].innerHTML).toEqual(
          "2,352.5",
          "with correct hover labels"
        );
      })
...
describe("hover after resizing", function() {
  ...
  it("should work", function(done) {
    ...
    Plotly.plot(gd, data, layout)
      ...
      .then(function() {
        return assertLabelCount(
          pos1,
          0,
          "before resize, not showing blank spot"
        );
      })
      .then(function() {
      ...
      .then(function() {
        return assertLabelCount(
          pos0,
          0,
          "after resize, not showing blank spot"
        );
      })
      .then(function() {
      ...
      .then(function() {
        return assertLabelCount(
          pos1,
          0,
          "back to initial, not showing blank spot"
        );
      })
      .then(done);
  });
...
describe("hover on fill", function() {
    ...
    return new Promise(function(resolve) {
      ...
      setTimeout(
        function() {
          ...
          expect(+transformCoords[0]).toBeCloseTo(
            labelPos[0],
            -1,
            labelText + ":x"
          );
          expect(+transformCoords[1]).toBeCloseTo(
            labelPos[1],
            -1,
            labelText + ":y"
          );
  ...
  it("should always show one label in the right place", function(done) {
    var mock = Lib.extendDeep(
      {},
      require("@mocks/scatter_fill_self_next.json")
    );
