...
var convert = require('../../../src/traces/densitymaplibre/convert');
...
describe('Test densitymaplibre defaults:', function() {
    var gd;
    var fullData;
    ...
    function _supply(opts, layout) {
        ...
        gd.data = opts.map(function(o) {
            return Lib.extendFlat({type: 'densitymaplibre'}, o || {});
        });
        gd.layout = layout || {};
...
describe('Test densitymaplibre convert:', function() {
    var base = function() {
        return {
            lon: [10, 20, 30],
            lat: [15, 25, 35],
    ...
    function pre(trace, layout) {
        var gd = {data: [Lib.extendFlat({type: 'densitymaplibre'}, trace)]};
...
describe('Test densitymaplibre hover:', function() {
    var gd;
    ...
    function run(s, done) {
        ...
        var fig = Lib.extendDeep({},
            s.mock || require('../../image/mocks/maplibre_density0.json')
        );
...
describe('Test densitymaplibre interactions:', function() {
    var gd;
    ...
    it('@gl should be able to restyle from and to *scattermaplibre*', function(done) {
        function _assert(msg, exp) {
            var traceHash = gd._fullLayout.maplibre._subplot.traceHash;
        ...
        .then(function() { _assert('after restyle to scattermaplibre', 'scattermaplibre'); })
        .then(function() { return Plotly.restyle(gd, 'type', 'densitymaplibre'); })
        .then(function() { _assert('back to densitymaplibre', 'densitymaplibre'); })
        .then(done, done.fail);
    }, 5 * jasmine.DEFAULT_TIMEOUT_INTERVAL);
});
