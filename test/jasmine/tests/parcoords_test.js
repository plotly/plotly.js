...
describe('parcoords initialization tests', function() {
  ...
  describe('parcoords defaults', function() {
    function _supply(traceIn) {
      var traceOut = { visible: true }, defaultColor = '#444', layout = {};
    ...
    it("'dimension' should be used with default values where attributes are not provided", function() {
      ...
      expect(fullTrace.dimensions).toEqual([
        { values: [1], visible: true, tickformat: '3s', _index: 0 },
      ]);
    });
...
describe('@noCI parcoords', function() {
  beforeAll(function() {
    ...
    mock.data[0].line.color = mock.data[0].line.color.slice(
      lineStart,
      lineStart + lineCount
    );
  });
  ...
  describe('edge cases', function() {
    ...
    it('Do something sensible if there is no panel i.e. dimension count is less than 2', function(
      done
    ) {
    ...
    it('Works with duplicate dimension labels', function(done) {
      ...
      mockCopy.data[0].dimensions[1].label =
        mockCopy.data[0].dimensions[0].label;
    ...
    it('Works with a single line; also, use a longer color array than the number of lines', function(
      done
    ) {
      var mockCopy = Lib.extendDeep({}, mock2);
      var dim, i, j;
    ...
    it('Does not raise an error with zero lines and no specified range', function(
      done
    ) {
      var mockCopy = Lib.extendDeep({}, mock2);
      var dim, i;
    ...
    it('Works with non-finite `values` elements', function(done) {
      ...
      Plotly.plot(gd, mockCopy.data, mockCopy.layout).then(function() {
        ...
        expect(gd.data[0].dimensions.length).toEqual(2);
        expect(document.querySelectorAll('.axis').length).toEqual(2);
        expect(gd.data[0].dimensions[0].values.length).toEqual(
          values[0].length
        );
        done();
      });
    ...
    it('Truncates dimension values to the shortest array, retaining only 3 lines', function(
      done
    ) {
      var mockCopy = Lib.extendDeep({}, mock1);
      var newDimension, i, j;
    ...
    it('Skip dimensions which are not plain objects or whose `values` is not an array', function(
      done
    ) {
      var mockCopy = Lib.extendDeep({}, mock1);
      var newDimension, i, j;
  ...
  describe('basic use', function() {
    ...
    it('`Plotly.plot` should have proper fields on `gd.data` on initial rendering', function() {
      ...
      expect(gd.data[0].dimensions[0].range).not.toBeDefined();
      expect(gd.data[0].dimensions[0].constraintrange).toBeDefined();
      expect(gd.data[0].dimensions[0].constraintrange).toEqual([
        100000,
        150000,
      ]);
      expect(gd.data[0].dimensions[1].range).toBeDefined();
    ...
    it('Calling `Plotly.plot` again should add the new parcoords', function(
      done
    ) {
      var reversedMockCopy = Lib.extendDeep({}, mockCopy);
      reversedMockCopy.data[0].dimensions = reversedMockCopy.data[0].dimensions
        .slice()
        .reverse();
      ...
      Plotly.plot(
        gd,
        reversedMockCopy.data,
        reversedMockCopy.layout
      ).then(function() {
        ...
        expect(gd.data[1].dimensions[10].constraintrange).toEqual([
          100000,
          150000,
        ]);
    ...
    it('Calling `Plotly.restyle` with a string path should amend the preexisting parcoords', function(
      done
    ) {
      expect(gd.data.length).toEqual(1);
    ...
    it('Calling `Plotly.restyle` for a dimension should amend the preexisting dimension', function(
      done
    ) {
      function restyleDimension(key, setterValue) {
        ...
        return function() {
          return Plotly.restyle(
            ...
            expect(gd.data[0].dimensions[2][key]).toEqual(
              value,
              "for dimension attribute '" + key + "'"
            );
          });
        };
      }
      ...
      restyleDimension('label', 'new label')()
        ...
        .then(
          restyleDimension('ticktext', [
            ['alpha', 'gamma', 'beta', 'omega', 'epsilon'],
          ])
        )
        ...
        .then(
          restyleDimension('values', [[0, 0.1, 0.4, 1, 2, 0, 0.1, 0.4, 1, 2]])
        )
        .then(restyleDimension('visible', false))
        .then(done);
    });

    it('Calling `Plotly.restyle` with an object should amend the preexisting parcoords', function(
      done
    ) {
    ...
    it("Should emit a 'plotly_restyle' event", function(done) {
      ...
      Plotly.restyle(gd, 'line.colorscale', 'Viridis').then(
    ...
    it('Calling `Plotly.relayout` with string should amend the preexisting parcoords', function(
      done
    ) {
    ...
    it('Calling `Plotly.relayout`with object should amend the preexisting parcoords', function(
      done
    ) {
      expect(gd.layout.width).toEqual(1184);
  ...
  describe('Lifecycle methods', function() {
    ...
    it('Plotly.deleteTraces with two traces removes the deleted plot', function(
      done
    ) {
      var gd = createGraphDiv();
      var mockCopy = Lib.extendDeep({}, mock);
      ...
      Plotly.plot(gd, mockCopy)
        ...
        .then(function() {
          expect(
            document.querySelectorAll('.parcoords-line-layers').length
          ).toEqual(1);
    ...
    it('Calling `Plotly.restyle` with zero panels left should erase lines', function(
      done
    ) {
      var mockCopy = Lib.extendDeep({}, mock2);
      var gd = createGraphDiv();
      ...
      function restyleDimension(key, dimIndex, setterValue) {
        ...
        return function() {
          return Plotly.restyle(
            gd,
            'dimensions[' + dimIndex + '].' + key,
            setterValue
          ).then(function() {
            expect(gd.data[0].dimensions[dimIndex][key]).toEqual(
              value,
              "for dimension attribute '" + key + "'"
            );
          });
      ...
      restyleDimension('values', 1, [[]])().then(function() {
        d3.selectAll('.parcoords-lines').each(function(d) {
          var imageArray = d.lineLayer.readPixels(
            0,
            0,
            d.model.canvasWidth,
    ...
    describe('Having two datasets', function() {
      it('Two subsequent calls to Plotly.plot should create two parcoords rows', function(
        done
      ) {
        var gd = createGraphDiv();
        var mockCopy = Lib.extendDeep({}, mock);
      ...
      it('Plotly.restyle should update the existing parcoords row', function(
        done
      ) {
        var gd = createGraphDiv();
        var mockCopy = Lib.extendDeep({}, mock);
        ...
        mockCopy2.data[0].dimensions[2].values = mockCopy2.data[0].dimensions[
          2
        ].values.map(numberUpdater);
