...
describe('spikeline search', function () {
    ...
    // The search for a point to spike to is expensive when spikedistance is -1,
    // because every point in the graph must be checked (with the current implementation).
    // These tests check that the search is only done when necessary.
    // We check the number of calls to hoverPoints to determine whether the search was done,
    // because the actual search is done inside hoverPoints.
    // One call to hoverPoints is expected no matter what; an additional call should be made
    // only if an axis is set to show spikes and snap to data or cursor.
    // Note: If the implementation changes in the future to avoid calling hoverPoints,
