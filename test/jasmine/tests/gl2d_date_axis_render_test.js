...
describe("date axis", function() {
  ...
  it("should use the fancy gl-vis/gl-scatter2d", function() {
    ...
    expect(gd._fullLayout._plots.xy._scene2d.glplot.objects[3].pointCount).toBe(
      0
    );
  });
    ...
    PlotlyInternal.plot(gd, [
      {
        type: "scattergl",
        mode: "markers",
        // important, as otherwise lines are assumed (which needs fancy)
        x: [new Date("2016-10-10"), new Date("2016-10-11")],
        y: [15, 16]
      }
    ]);
  ...
  it(
    "should use the non-fancy gl-vis/gl-scatter2d with string dates",
    function() {
      PlotlyInternal.plot(gd, [
        {
          type: "scattergl",
      ...
      expect(
        gd._fullLayout._plots.xy._scene2d.glplot.objects[3].pointCount
      ).toBe(2);
    }
  );
});
