...
describe("Test gl plot interactions", function() {
  ...
  afterEach(function() {
    var fullLayout = gd._fullLayout, sceneIds;
  ...
  describe("gl3d plots", function() {
    ...
    describe("scatter3d hover", function() {
      ...
      it("should have", function() {
        ...
        expect(Object.keys(ptData)).toEqual(
          ["x", "y", "z", "data", "fullData", "curveNumber", "pointNumber"],
    ...
    it("should be able to reversibly change trace type", function(done) {
      ...
      Plotly.restyle(gd, "type", "scatter")
        .then(function() {
  ...
  describe("gl2d plots", function() {
    var mock = require("@mocks/gl2d_10.json"), modeBar, relayoutCallback;
    ...
    it("should respond to drag interactions", function(done) {
      ...
      // Switch to pan mode
      expect(buttonPan.isActive()).toBe(false);
      // initially, zoom is active
      buttonPan.click();
      expect(buttonPan.isActive()).toBe(true);

      // switched on dragmode
  ...
  describe("gl3d event handlers", function() {
    ...
    describe("modebar click handlers", function() {
      it(
        "button zoom3d should updates the scene dragmode and dragmode button",
        function() {
      ...
      it(
        "button pan3d should updates the scene dragmode and dragmode button",
        function() {
          var buttonTurntable = selectButton(modeBar, "tableRotation"),
      ...
      it(
        "button orbitRotation should updates the scene dragmode and dragmode button",
        function() {
          var buttonTurntable = selectButton(modeBar, "tableRotation"),
      ...
      it(
        "button hoverClosest3d should update the scene hovermode and spikes",
        function() {
      ...
      it("button resetCameraDefault3d should reset camera to default", function(
        done
      ) {
        var buttonDefault = selectButton(modeBar, "resetCameraDefault3d");

        expect(gd._fullLayout.scene._scene.cameraInitial.eye).toEqual({
          x: 0.1,
          y: 0.1,
          z: 1
        });
        expect(gd._fullLayout.scene2._scene.cameraInitial.eye).toEqual({
          x: 2.5,
          y: 2.5,
          z: 2.5
        });
        ...
        gd.once("plotly_relayout", function() {
          ...
          expect(gd._fullLayout.scene._scene.getCamera().eye.z).toBeCloseTo(
            1.25
          );
          expect(gd._fullLayout.scene2._scene.getCamera().eye.z).toBeCloseTo(
            1.25
          );
      ...
      it(
        "button resetCameraLastSave3d should reset camera to default",
        function(done) {
          var buttonDefault = selectButton(modeBar, "resetCameraDefault3d");
    ...
    describe("drag and wheel interactions", function() {
      it("should update the scene camera", function(done) {
        var sceneLayout = gd._fullLayout.scene,
          ...
          sceneTarget = gd.querySelector(
            ".svg-container .gl-container #scene  canvas"
          ),
          sceneTarget2 = gd.querySelector(
            ".svg-container .gl-container #scene2 canvas"
          );

        expect(sceneLayout.camera.eye).toEqual({ x: 0.1, y: 0.1, z: 1 });
        expect(sceneLayout2.camera.eye).toEqual({ x: 2.5, y: 2.5, z: 2.5 });
        ...
        setTimeout(
          function() {
            ...
            // Drag scene 1
            sceneTarget.dispatchEvent(new MouseEvent("mousedown", {
              x: 0,
              y: 0
            }));
            sceneTarget.dispatchEvent(new MouseEvent("mousemove", {
              x: 100,
              y: 100
            }));
            sceneTarget.dispatchEvent(new MouseEvent("mouseup", {
              x: 100,
              y: 100
            }));

            // Drag scene 2
            sceneTarget2.dispatchEvent(new MouseEvent("mousedown", {
              x: 0,
              y: 0
            }));
            sceneTarget2.dispatchEvent(new MouseEvent("mousemove", {
              x: 100,
              y: 100
            }));
            sceneTarget2.dispatchEvent(new MouseEvent("mouseup", {
              x: 100,
              y: 100
            }));
  ...
  describe("Removal of gl contexts", function() {
    var mockData2d = [{ type: "scattergl", x: [1, 2, 3], y: [2, 1, 3] }];

    var mockData3d = [
      { type: "scatter3d", x: [1, 2, 3], y: [2, 1, 3], z: [3, 2, 1] }
    ];
    ...
    describe("Plotly.newPlot", function() {
      var mockData2dNew = [{ type: "scattergl", x: [1, 3, 2], y: [2, 3, 1] }];

      var mockData3dNew = [
        { type: "scatter3d", x: [2, 1, 3], y: [1, 2, 3], z: [2, 1, 3] }
      ];
...
describe("Test gl plot side effects", function() {
  ...
  describe("when present with rangeslider", function() {
    it("should not draw the rangeslider", function(done) {
      var data = [
        { x: [1, 2, 3], y: [2, 3, 4], type: "scattergl" },
        { x: [1, 2, 3], y: [2, 3, 4], type: "scatter" }
      ];

      var layout = { xaxis: { rangeslider: { visible: true } } };
  ...
  it("should be able to replot from a blank graph", function(done) {
    ...
    var data = [{ type: "scattergl", x: [1, 2, 3], y: [2, 1, 2] }];
