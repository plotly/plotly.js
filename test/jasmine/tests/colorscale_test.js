...
describe("Test colorscale:", function() {
  ...
  describe("isValidScale", function() {
    var isValidScale = Colorscale.isValidScale, scl;
    ...
    it(
      "should accept only arrays with 1st val = 0 and last val = 1",
      function() {
  ...
  describe("flipScale", function() {
    var flipScale = Colorscale.flipScale, scl;
  ...
  describe("hasColorscale", function() {
    var hasColorscale = Colorscale.hasColorscale, trace;

    it("should return false when marker is not defined", function() {
      var shouldBeFalse = [{}, { marker: null }];
    ...
    it(
      "should return false when marker is not defined (nested version)",
      function() {
        var shouldBeFalse = [{}, { marker: null }, { marker: { line: null } }];
        shouldBeFalse.forEach(function(trace) {
    ...
    it(
      "should return true when marker color is an Array with at least one number",
      function() {
        trace = {
          marker: { color: [1, 2, 3], line: { color: [2, 3, 4] } }
        };
        ...
        trace = {
          marker: {
            ...
            line: { color: ["rgb(100, 100, 100)", "#d0d0d0", "#fff"] }
          }
        };
        expect(hasColorscale(trace, "marker")).toBe(false);
        expect(hasColorscale(trace, "marker.line")).toBe(false);
    ...
    it("should return true when marker showscale is true", function() {
      trace = { marker: { showscale: true, line: { showscale: true } } };
      expect(hasColorscale(trace, "marker")).toBe(true);
      expect(hasColorscale(trace, "marker.line")).toBe(true);
    });
    ...
    it("should return true when marker colorscale is valid", function() {
      trace = {
        marker: {
          colorscale: "Greens",
          line: { colorscale: [[0, "rgb(0,0,0)"], [1, "rgb(0,0,0)"]] }
        }
      };
      expect(hasColorscale(trace, "marker")).toBe(true);
      expect(hasColorscale(trace, "marker.line")).toBe(true);
    ...
    it("should return true when marker cmin & cmax are numbers", function() {
      trace = { marker: { cmin: 10, cmax: 20, line: { cmin: 10, cmax: 20 } } };
      expect(hasColorscale(trace, "marker")).toBe(true);
      expect(hasColorscale(trace, "marker.line")).toBe(true);
    });

    it("should return true when marker colorbar is defined", function() {
      trace = { marker: { colorbar: {}, line: { colorbar: {} } } };
      expect(hasColorscale(trace, "marker")).toBe(true);
      expect(hasColorscale(trace, "marker.line")).toBe(true);
    });
  });
  ...
  describe("handleDefaults (heatmap-like version)", function() {
    ...
    it("should coerce autocolorscale to false unless set to true", function() {
      ...
      traceIn = { colorscale: "Greens" };
      handleDefaults(traceIn, traceOut, layout, coerce, opts);
      expect(traceOut.autocolorscale).toBe(false);

      traceIn = { autocolorscale: true };
      handleDefaults(traceIn, traceOut, layout, coerce, opts);
  ...
  describe("handleDefaults (scatter-like version)", function() {
    ...
    it(
      "should coerce autocolorscale to false when valid colorscale is given",
      function() {
        traceIn = { marker: { colorscale: "Greens" } };
        handleDefaults(traceIn, traceOut, layout, coerce, opts);
        expect(traceOut.marker.autocolorscale).toBe(false);

        traceIn = { marker: { colorscale: "nope" } };
        handleDefaults(traceIn, traceOut, layout, coerce, opts);
        expect(traceOut.marker.autocolorscale).toBe(true);
      }
    );
  ...
  describe("calc", function() {
    ...
    it(
      "should be reverse the auto scale when reversescale is true",
      function() {
        trace = {
          type: "heatmap",
          z: [["a", "b"], [0.5, "d"]],
        ...
        calcColorscale(trace, z, "", "z");
        expect(trace.autocolorscale).toBe(true);
        expect(trace.colorscale[trace.colorscale.length - 1]).toEqual([
          1,
          "rgb(220,220,220)"
        ]);
      }
