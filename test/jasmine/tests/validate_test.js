...
describe("Plotly.validate", function() {
  ...
  it("should return undefined when no errors are found", function() {
    var out = Plotly.validate([{ type: "scatter", x: [1, 2, 3] }], {
  ...
  it("should report when a data trace is not an object", function() {
    var out = Plotly.validate([{ type: "bar", x: [1, 2, 3] }, [1, 2, 3]]);
  ...
  it(
    "should report when trace contains keys not part of the schema",
    function() {
      var out = Plotly.validate([{ x: [1, 2, 3], markerColor: "blue" }], {});
  ...
  it("should report when trace contains keys that are not coerced", function() {
    var out = Plotly.validate(
      [
        { x: [1, 2, 3], mode: "lines", marker: { color: "blue" } },
        { x: [1, 2, 3], mode: "markers", marker: { color: "blue", cmin: 10 } }
      ],
      {}
    );
  ...
  it(
    "should report when trace contains keys set to invalid values",
    function() {
      var out = Plotly.validate(
        [
          { x: [1, 2, 3], mode: "lines", line: { width: "a big number" } },
          { x: [1, 2, 3], mode: "markers", marker: { color: 10 } }
        ],
        {}
      );
  ...
  it("should work with info arrays", function() {
    var out = Plotly.validate([{ y: [1, 2, 2] }], {
      xaxis: { range: [0, 10] },
  ...
  it("should work with isLinkedToArray attributes", function() {
    var out = Plotly.validate([], {
      ...
      xaxis: {
        ...
        rangeselector: {
          buttons: [
            { label: "1 month", step: "all", count: 10 },
            "wont-work",
            { title: "1 month" }
          ]
        }
      ...
      xaxis2: {
        type: "date",
        rangeselector: { buttons: [{ title: "1 month" }] }
      },
      xaxis3: { type: "date", rangeselector: { buttons: "wont-work" } },
      shapes: [{ opacity: "none" }],
      updatemenus: [
        { buttons: [{ method: "restyle", args: ["marker.color", "red"] }] },
        "wont-work",
        {
          buttons: [
            { method: "restyle", args: null },
            {
              method: "relayout",
  ...
  it("should work with attributes in registered transforms", function() {
    var base = { x: [-2, -1, -2, 0, 1, 2, 3], y: [1, 2, 3, 1, 2, 3, 1] };
      ...
      [
        Lib.extendFlat({}, base, {
          transforms: [
            ...
            { type: "filter", operation: "=", wrongKey: "sup?" },
            { type: "filter", operation: "wrongVal" },
            "wont-work"
          ]
        }),
        Lib.extendFlat({}, base, { transforms: { type: "filter" } }),
        Lib.extendFlat({}, base, { transforms: [{ type: "no gonna work" }] })
      ],
