...
describe("Test animate API", function() {
  ...
  it("throws an error on addFrames if gd is not a graph", function() {
    ...
    expect(function() {
      ...
      "This element is not a Plotly plot: [object HTMLDivElement]. It's likely that you've failed to create a plot before adding frames. For more details, see https://plot.ly/javascript/animations/"
    ));
    ...
    describe("With duration = " + duration, function() {
      ...
      it("animates to a frame", function(done) {
        Plotly.animate(gd, ["frame0"], {
          transition: { duration: 1.2345 },
          frame: { duration: 1.5678 }
        })
          .then(function() {
      ...
      it("treats objects as frames", function(done) {
        ...
        Plotly.animate(gd, frame, animOpts)
          .then(function() {
      ...
      it("treats a list of objects as frames", function(done) {
        ...
        Plotly.animate(gd, [frame1, frame2], animOpts)
          .then(function() {
      ...
      it("animates all frames if list is null", function(done) {
        Plotly.animate(gd, null, animOpts)
          .then(function() {
      ...
      it("animates all frames if list is undefined", function(done) {
        Plotly.animate(gd, undefined, animOpts)
          .then(function() {
            verifyFrameTransitionOrder(gd, [
              "base",
              "frame0",
      ...
      it("animates to a single frame", function(done) {
        Plotly.animate(gd, ["frame0"], animOpts)
          .then(function() {
            expect(Plots.transition.calls.count()).toEqual(1);
            verifyQueueEmpty(gd);
          })
      ...
      it("animates frames in the correct order", function(done) {
        Plotly.animate(gd, ["frame0", "frame2", "frame1", "frame3"], animOpts)
          .then(function() {
            verifyFrameTransitionOrder(gd, [
              "frame0",
              "frame2",
      ...
      it(
        "falls back to animationOpts[0] if not enough supplied in array",
        function(done) {
          Plotly.animate(gd, ["frame0", "frame1"], {
      ...
      it("emits plotly_animated before the promise is resolved", function(
        done
      ) {
        var animated = false;
      ...
      it(
        "emits plotly_animated as each animation in a sequence completes",
        function(done) {
          var completed = 0;
          var test1 = 0, test2 = 0;
          gd.on("plotly_animated", function() {
  ...
  describe("Animation direction", function() {
    ...
    beforeEach(function() {
      animOpts = { frame: { duration: 0 }, transition: { duration: 0 } };
    });
  ...
  describe("Animation fromcurrent", function() {
    ...
    it("animates starting at the current frame", function(done) {
      Plotly.animate(gd, ["frame1"], animOpts)
        .then(function() {
    ...
    it(
      "plays in reverse from the end when current frame = first frame",
      function(done) {
        animOpts.direction = "reverse";

        Plotly.animate(gd, ["base"], animOpts)
          .then(function() {
  ...
  describe("Calling Plotly.animate synchronously in series", function() {
    ...
    it(
      "emits plotly_animationinterrupted when an animation is interrupted",
      function(done) {
        ...
        Plotly.animate(
          gd,
          ["frame2"],
          Lib.extendFlat(animOpts, { mode: "immediate" })
        )
          .then(function() {
    ...
    it("queues successive animations", function(done) {
      ...
      Plotly.animate(gd, "odd-frames", { transition: { duration: 16 } })
        .then(delay(10))
        .then(function() {
    ...
    it("animates groups in the correct order", function(done) {
      Plotly.animate(gd, "even-frames", animOpts);
      Plotly.animate(gd, "odd-frames", animOpts)
        .then(function() {
          verifyFrameTransitionOrder(gd, [
            "frame0",
            "frame2",
  ...
  describe("frame vs. transition timing", function() {
    ...
    it(
      "limits the transition duration to <= frame duration (matching per-config)",
      function(done) {
        Plotly.animate(gd, ["frame0", "frame1"], {
...
describe("Animate API details", function() {
  ...
  it("redraws after a layout animation", function(done) {
    ...
    Plotly.animate(gd, { layout: { "xaxis.range": [0, 1] } }, {
      frame: { redraw: true, duration: dur },
  ...
  it("forces a relayout after layout animations", function(done) {
    ...
    Plotly.animate(gd, { layout: { "xaxis.range": [0, 1] } }, {
      frame: { redraw: false, duration: dur },
      transition: { duration: dur }
    })
      .then(function() {
  ...
  it("triggers plotly_animated after a single layout animation", function(
    done
  ) {
    var animateds = 0;
    gd.on("plotly_animated", function() {
  ...
  it("triggers plotly_animated after a multi-step layout animation", function(
    done
  ) {
    var animateds = 0;
    gd.on("plotly_animated", function() {
  ...
  it("ignores null and undefined frames", function(done) {
    ...
    Plotly.addFrames(gd, mockCopy.frames)
      .then(function() {
        return Plotly.animate(gd, ["frame0", null, undefined], {
          transition: { duration: 0 },
          frame: { duration: 0 }
        });
      })
...
describe("non-animatable fallback", function() {
  ...
  it("falls back to a simple update for bar graphs", function(done) {
    Plotly.plot(gd, [{ x: [1, 2, 3], y: [4, 5, 6], type: "bar" }])
      .then(function() {
        expect(gd.data[0].y).toEqual([4, 5, 6]);

        return Plotly.animate(gd, [{ data: [{ y: [6, 4, 5] }] }], {
          frame: { duration: 0 }
        });
      })
      .then(function() {
...
describe("animating scatter traces", function() {
  ...
  it("animates trace opacity", function(done) {
    var trace;
    Plotly.plot(gd, [{ x: [1, 2, 3], y: [4, 5, 6], opacity: 1 }])
      .then(function() {
        trace = Plotly.d3.selectAll("g.scatter.trace");
        expect(trace.style("opacity")).toEqual("1");

        return Plotly.animate(gd, [{ data: [{ opacity: 0.1 }] }], {
          transition: { duration: 0 },
