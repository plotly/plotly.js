...
describe('annotations relayout', function() {
    ...
    Plotly.relayout(gd, 'annotations[' + len + ']', ann)
      ...
      .then(function() {
        ...
        return Plotly.relayout(gd, {
          annotations: [ann, { text: 'boo', x: 1, y: 1 }],
        });
      })
      .then(function() {
  ...
  it('can update several annotations and add and delete in one call', function(
    done
  ) {
    expect(countAnnotations()).toEqual(len);
    ...
    Plotly.relayout(gd, {
      ...
      .then(function() {
        ...
        expect(fullAnnosAfter[3]._cheese).toEqual([
          'gorgonzola',
          'gouda',
          'gloucester',
        ]);
  ...
  [
    ...
    it(
      'warns on ambiguous combinations and invalid values: ' +
  ...
  it('keeps the same data value if the axis type is changed without position', function(
    ...
    Plotly.relayout(gd, { 'yaxis.type': 'log' })
      .then(function() {
        ...
        expect(gd.layout.annotations[1].ay).toBeCloseTo(
          Math.LN2 / Math.LN10,
          6
        );
...
describe('annotations autorange', function() {
  ...
  function assertRanges(x, y, x2, y2, x3, y3) {
    ...
    expect(Lib.simpleMap(dateAx.range, dateAx.r2l)).toBeCloseToArray(
      Lib.simpleMap(x2, dateAx.r2l),
      PRECX2,
      'xaxis2 ' + dateAx.range
    );
    ...
    Plotly.plot(gd, mock)
      ...
      .then(function() {
        assertRanges(
          [1.44, 2.02],
          [0.99, 1.52],
          ['2001-01-31 23:59:59.999', '2001-02-01 00:00:00.001'],
          [-0.069, 3.917],
          [0.5, 2.5],
          [0.92, 2.08]
        );
...
describe('annotation clicktoshow', function() {
  ...
  function clickAndCheck(opts) {
    return function() {
      expect(Annotations.hasClickToShow(gd, hoverData(opts.newPts))).toBe(
        opts.newCTS,
        'step: ' + opts.step
      );
  ...
  it('should select only clicktoshow annotations matching x, y, and axes of any point', function(
    done
  ) {
    ...
    Plotly.plot(gd, data, layout())
      ...
      .then(
        updateAndCheck({
          update: allAnnotations('visible', false),
          on: [],
          step: 2,
        })
      )
      ...
      .then(
        updateAndCheck({
          update: allAnnotations('clicktoshow', 'onout'),
          on: [],
          step: 4,
        })
      )
      ...
      .then(
        updateAndCheck({
          update: allAnnotations('clicktoshow', 'onoff'),
          on: [],
          step: 10,
        })
      )
      ...
      .then(
        clickAndCheck({ newPts: [[1, 2]], newCTS: true, on: [2], step: 15 })
      )
  ...
  it('works on date and log axes', function(done) {
    Plotly.plot(
      ...
      .then(
        clickAndCheck({ newPts: [['2016-01-02', 1]], newCTS: true, on: [0] })
      )
      .catch(failTest)
      .then(done);
  });
  ...
  it('works on category axes', function(done) {
    Plotly.plot(
      ...
      .then(
        clickAndCheck({ newPts: [['b', 2]], newCTS: true, on: [0], step: 1 })
      )
      .then(
        clickAndCheck({ newPts: [['c', 3]], newCTS: true, on: [1], step: 2 })
      )
      .catch(failTest)
      .then(done);
  });
...
describe('annotation effects', function() {
  ...
  function checkDragging(findDragger, autoshiftX, autoshiftY, coordScale) {
    ...
    return dragAndReplot(findDragger(), 30, -30)
      ...
      .then(function() {
        var bbox = textBox().getBoundingClientRect();
        expect(bbox.left).toBeWithin(
          bboxInitial.left + 240 - 2 * autoshiftX,
          2
        );
  ...
  function checkTextDrag() {
    ...
    return dragAndReplot(textDrag(), 50, -50).then(function() {
      var bbox = textBox().getBoundingClientRect();
  ...
  it('respects anchor: auto when paper-referenced without arrow', function(
    done
  ) {
    makePlot([
      {
  ...
  it('also works paper-referenced with explicit anchors and no arrow', function(
    done
  ) {
    makePlot([
      {
  ...
  it('should only make the clippaths it needs and delete others', function(
    done
  ) {
    makePlot([
  ...
  it('should register clicks and show hover effects on the text box only', function(
    done
  ) {
    ...
    makePlot(
      ...
      .then(function() {
        ...
        return assertHoverLabels(
          [[pos0, 'bananas'], [pos1, ''], [pos2, '']],
          '0 only'
        );
      })
      ...
      .then(function() {
        ...
        return assertHoverLabels(
          [[pos0, 'bananas'], [pos1, 'chicken'], [pos2, '']],
          '0 and 1'
        );
      })
      .catch(failTest)
