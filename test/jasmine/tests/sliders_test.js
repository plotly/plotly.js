...
describe("sliders defaults", function() {
  ...
  it("should set 'visible' to false when no steps are present", function() {
    layoutIn.sliders = [
      {
        steps: [
          { method: "relayout", args: ["title", "Hello World"] },
          {
            method: "update",
      ...
      {
        visible: false,
        steps: [{ method: "relayout", args: ["title", "Hello World"] }]
      }
    ];
  ...
  it(
    "should not coerce currentvalue defaults unless currentvalue is visible",
    function() {
      layoutIn.sliders = [
        {
          currentvalue: { visible: false, xanchor: "left" },
          steps: [
  ...
  it("should set the default values equal to the labels", function() {
    layoutIn.sliders = [
      {
        steps: [
          { method: "relayout", args: [], label: "Label #1", value: "label-1" },
          { method: "update", args: [], label: "Label #2" },
          { method: "animate", args: [], value: "lacks-label" }
        ]
      }
    ];
    ...
    expect(layoutOut.sliders[0].steps).toEqual([
      { method: "relayout", args: [], label: "Label #1", value: "label-1" },
      { method: "update", args: [], label: "Label #2", value: "Label #2" },
      { method: "animate", args: [], label: "step-2", value: "lacks-label" }
    ]);
  });
  ...
  it("adds and removes slider steps gracefully", function(done) {
    ...
    Plotly.relayout(gd, { "sliders[0].active": 2 })
      .then(function() {
        ...
        gd.layout.sliders[0].steps.push({
          method: "restyle",
          args: [],
          label: "first"
        });
...
describe("sliders interactions", function() {
  ...
  it("should respond to mouse clicks", function(done) {
    var firstGroup = gd._fullLayout._infolayer.select(
      "." + constants.railTouchRectClass
    );
    var firstGrip = gd._fullLayout._infolayer.select(
      "." + constants.gripRectClass
    );
  ...
  it("should issue events on interaction", function(done) {
    ...
    gd
      .on("plotly_sliderstart", function() {
        cntStart++;
      })
    ...
    function assertEventCounts(starts, interactions, noninteractions, ends) {
      expect([cntStart, cntInteraction, cntNonInteraction, cntEnd]).toEqual([
        starts,
        interactions,
        noninteractions,
        ends
