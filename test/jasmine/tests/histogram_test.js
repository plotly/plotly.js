...
describe("Test histogram", function() {
  ...
  describe("supplyDefaults", function() {
    ...
    it(
      "should set visible to false when type is histogram2d and x or y are empty",
      function() {
        traceIn = { type: "histogram2d", x: [], y: [1, 2, 2] };
        supplyDefaults(traceIn, traceOut, "", {});
        expect(traceOut.visible).toBe(false);

        traceIn = { type: "histogram2d", x: [1, 2, 2], y: [] };
        supplyDefaults(traceIn, traceOut, "", {});
        expect(traceOut.visible).toBe(false);

        traceIn = { type: "histogram2d", x: [], y: [] };
        supplyDefaults(traceIn, traceOut, "", {});
        expect(traceOut.visible).toBe(false);

        traceIn = { type: "histogram2dcontour", x: [], y: [1, 2, 2] };
        supplyDefaults(traceIn, traceOut, "", {});
        expect(traceOut.visible).toBe(false);
      }
    );
    ...
    it("should not coerce autobinx regardless of xbins", function() {
      traceIn = { x: [1, 2, 2], xbins: { start: 1, end: 3, size: 1 } };
      supplyDefaults(traceIn, traceOut, "", {});
    ...
    it("should not coerce autobiny regardless of ybins", function() {
      traceIn = { y: [1, 2, 2], ybins: { start: 1, end: 3, size: 1 } };
      supplyDefaults(traceIn, traceOut, "", {});
    ...
    it("should take its own calendars", function() {
      traceIn = { x: [1, 2, 3], xcalendar: "coptic", ycalendar: "nepali" };
      supplyDefaults(traceIn, traceOut, "", { calendar: "islamic" });
  ...
  describe("calc", function() {
    ...
    it("should handle auto dates with uniform (day) bins", function() {
      ...
      var x0 = 0, x1 = x0 + oneDay, x2 = x1 + oneDay, x3 = x2 + oneDay;
    ...
    describe("cumulative distribution functions", function() {
      ...
      var CDFs = [
        ...
        { direction: "decreasing", p: [2, 7, 12, 17], s: [10, 9, 7, 4] },
        { currentbin: "exclude", p: [7, 12, 17, 22], s: [1, 3, 6, 10] },
        {
        ...
        },
        {
          ...
          histnorm: "percent",
          p: [-3, 2, 7, 12, 17],
          s: [100, 3100 / 32, 2700 / 32, 1900 / 32, 700 / 32]
        },
        {
          direction: "decreasing",
          currentbin: "half",
