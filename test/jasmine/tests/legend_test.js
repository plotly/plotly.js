...
describe('legend defaults', function () {
    ...
    it('does not show with a hidden pie', function () {
        fullData = allShown([{ type: 'pie', showlegend: false }]);
    ...
    it('shows if even a default hidden single item is explicitly shown', function () {
        fullData = allShown([{ type: 'contour', _dfltShowLegend: false, _input: { showlegend: true } }]);
    ...
    it('should default traceorder to reversed for filled tonext scatter charts', function () {
        fullData = allShown([{ type: 'scatter' }, { type: 'scatter', fill: 'tonexty' }]);
    ...
    it('should default traceorder to grouped when a group is present', function () {
        fullData = allShown([{ type: 'scatter', legendgroup: 'group' }, { type: 'scatter' }]);
    ...
    it('does not consider invisible traces for traceorder default', function () {
        fullData = allShown([{ type: 'bar', visible: false }, { type: 'bar', visible: false }, { type: 'scatter' }]);
        ...
        fullData = allShown([{ type: 'scatter', legendgroup: 'group', visible: false }, { type: 'scatter' }]);
...
describe('legend anchor utils:', function () {
    ...
    describe('isRightAnchor', function () {
        ...
        it("should return true when 'xanchor' is set to 'auto' and 'x' >= 2/3", function () {
            ...
            [0, 0.4, 0.7, 1].forEach(function (v) {
                opts.x = v;
                expect(isRightAnchor(opts)).toBe(v > threshold, 'case ' + v);
            });
        });
    });
    ...
    describe('isCenterAnchor', function () {
        ...
        it("should return true when 'xanchor' is set to 'auto' and 1/3 < 'x' < 2/3", function () {
            ...
            [0, 0.4, 0.7, 1].forEach(function (v) {
                opts.x = v;
                expect(isCenterAnchor(opts)).toBe(v > threshold0 && v < threshold1, 'case ' + v);
            });
        });
    });
    ...
    describe('isBottomAnchor', function () {
        ...
        it("should return true when 'yanchor' is set to 'auto' and 'y' <= 1/3", function () {
            ...
            [0, 0.4, 0.7, 1].forEach(function (v) {
                opts.y = v;
                expect(isBottomAnchor(opts)).toBe(v < threshold, 'case ' + v);
            });
        });
    });
    ...
    describe('isMiddleAnchor', function () {
        ...
        it("should return true when 'yanchor' is set to 'auto' and 1/3 < 'y' < 2/3", function () {
            ...
            [0, 0.4, 0.7, 1].forEach(function (v) {
                opts.y = v;
                expect(isMiddleAnchor(opts)).toBe(v > threshold0 && v < threshold1, 'case ' + v);
            });
        });
    });
});
...
describe('legend relayout update', function () {
    ...
    it('should make legend fit in graph viewport', function (done) {
        ...
        Plotly.newPlot(gd, fig)
            ...
            .then(function () {
                return Plotly.relayout(gd, 'legend.x', 0.8);
            })
    ...
    it('should fit in graph viewport when changing legend.title.side', function (done) {
        ...
        Plotly.newPlot(gd, fig)
            ...
            .then(function () {
                return Plotly.relayout(gd, 'legend.title.side', 'left');
            })
            .then(_assert('after relayout to *left*', [607.54, 60], [180, 67]))
            .then(function () {
                return Plotly.relayout(gd, 'legend.title.side', 'top');
            })
...
describe('legend interaction', function () {
    ...
    describe('staticPlot', function () {
        ...
        it('should prevent toggling if set', function (done) {
            ...
            Plotly.newPlot(gd, data, layout, config).then(toggleTrace).then(assertToggled(false)).then(done, done.fail);
        });
    });
    ...
    describe('visible toggle', function () {
        ...
        var data = [{ y: [1, 2, 1] }, { y: [2, 1, 2] }, { y: [2, 3, 4] }];
    ...
    describe('legend visibility interactions', function () {
        ...
        describe('legend click/doubleclick event data', function () {
            function _assert(act, exp) {
                ...
                expect(Object.keys(act).length).toBe(Object.keys(exp).length, '# of keys');
            }
        ...
        describe('should honor *itemclick* and *itemdoubleclick* settings', function () {
            ...
            function run() {
                return Promise.resolve()
                    .then(click(0, 1))
                    .then(_assert(['legendonly', true, true]))
                    .then(click(0, 1))
                    .then(_assert([true, true, true]))
                    .then(click(0, 2))
                    .then(_assert([true, 'legendonly', 'legendonly']))
                    .then(click(0, 2))
                    .then(_assert([true, true, true]))
                    .then(function () {
                        return Plotly.relayout(gd, {
            ...
            it(
                ...
                function (done) {
                    ...
                    Plotly.newPlot(gd, [{ y: [1, 2, 1] }, { y: [2, 1, 2] }, { y: [3, 5, 0] }])
                        .then(run)
                        .then(done, done.fail);
                },
        ...
        describe('should honor *itemclick* and *itemdoubleclick* settings | case of pie in multiple legends', function () {
            ...
            function run() {
                return Promise.resolve()
                    .then(click(0, 1))
                    .then(_assert(['legendonly', true, true, true, true, true]))
                    .then(click(0, 1))
                    .then(_assert([true, true, true, true, true, true]))
                    .then(click(0, 2))
                    .then(_assert([true, 'legendonly', 'legendonly', true, true, true]))
                    .then(click(0, 2))
                    .then(_assert([true, true, true, true, true, true]))
                    .then(function () {
                        return Plotly.relayout(gd, {
...
describe('legend with custom doubleClickDelay', function () {
    ...
    it(
        'should differentiate clicks and double-clicks according *doubleClickDelay* config',
        function (done) {
    ...
    it(
        'custom plotly_legenddoubleclick handler should fire even when plotly_legendclick has been cancelled',
        function (done) {
            var tShort = 0.75 * DBLCLICKDELAY;
            var dblClickCnt = 0;
            var newPlot = function (fig) {
...
describe('legend with custom legendwidth', function () {
    ...
    it('should change width when trace has legendwidth', function (done) {
        ...
        Plotly.newPlot(gd, { data: extendedData, layout: layout })
            .then(function () {
    ...
    it('should change width when legend has entrywidth', function (done) {
        ...
        Plotly.newPlot(gd, { data: data, layout: extendedLayout })
            .then(function () {
    ...
    it('should change group width when trace has legendwidth', function (done) {
        ...
        Plotly.newPlot(gd, { data: extendedData, layout: extendedLayout })
            .then(function () {
