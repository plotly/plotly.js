...
describe('relayout', function() {
  describe('axis category attributes', function() {
    ...
    it("should response to 'categoryarray' and 'categoryorder' updates", function(
      done
    ) {
...
describe('subplot creation / deletion:', function() {
  ...
  it('should clear orphan subplot when adding traces to blank graph', function(
    done
  ) {
  ...
  it('puts plot backgrounds behind everything except if they overlap', function(
    done
  ) {
    function checkBGLayers(behindCount, x2y2Count) {
      expect(gd.querySelectorAll('.bglayer rect.bg').length).toBe(behindCount);
      expect(gd.querySelectorAll('.subplot.x2y2 rect.bg').length).toBe(
        x2y2Count
      );
