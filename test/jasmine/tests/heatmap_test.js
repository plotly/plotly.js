...
describe("heatmap supplyDefaults", function() {
  ...
  it("should set visible to false when z is empty", function() {
    ...
    traceIn = { type: "heatmap", z: [[1, 2], []] };
    traceOut = Plots.supplyTraceDefaults(traceIn, 0, layout);

    traceIn = { type: "heatmap", z: [[], [1, 2], [1, 2, 3]] };
    traceOut = Plots.supplyTraceDefaults(traceIn, 0, layout);
  ...
  it("should set visible to false when z is non-numeric", function() {
    traceIn = { type: "heatmap", z: [["a", "b"], ["c", "d"]] };
    supplyDefaults(traceIn, traceOut, defaultColor, layout);
    expect(traceOut.visible).toBe(false);
  });

  it(
    "should set visible to false when z isn't column not a 2d array",
    function() {
      traceIn = {
        x: [1, 1, 1, 2, 2],
        y: [1, 2, 3, 1, 2],
  ...
  it("should set paddings to 0 when not defined", function() {
    traceIn = { type: "heatmap", z: [[1, 2], [3, 4]] };
  ...
  it("should not step on defined paddings", function() {
    traceIn = { xgap: 10, type: "heatmap", z: [[1, 2], [3, 4]] };
...
describe("heatmap convertColumnXYZ", function() {
  ...
  var xa = makeMockAxis(), ya = makeMockAxis();
  ...
  it(
    "should convert x/y/z columns to z(x,y) with uneven dimensions",
    function() {
      trace = {
  ...
  it(
    "should convert x/y/z columns to z(x,y) with out-of-order data",
    function() {
...
describe("heatmap calc", function() {
  ...
  it("should handle 1-xy + multi-brick case", function() {
    var out = _calc({ x: [2], y: [3], z: [[1, 2, 3], [3, 1, 2]] });
  ...
  it("should handle 0-xy + multi-brick case", function() {
    var out = _calc({ x: [], y: [], z: [[1, 2, 3], [3, 1, 2]] });
...
describe("heatmap plot", function() {
  ...
  it("draws canvas with correct margins", function(done) {
    var mockWithPadding = require("@mocks/heatmap_brick_padding.json"),
      ...
      gd = createGraphDiv(),
      getContextStub = { fillRect: jasmine.createSpy() },
    ...
    var argumentsWithoutPadding = [], argumentsWithPadding = [];
    Plotly.plot(gd, mockWithoutPadding.data, mockWithoutPadding.layout)
      .then(function() {
        argumentsWithoutPadding = getContextStub.fillRect.calls
          .allArgs()
          .slice(0);
      ...
      .then(function() {
        ...
        argumentsWithPadding = getContextStub.fillRect.calls
          .allArgs()
          .slice(getContextStub.fillRect.calls.allArgs().length - 9);
...
describe("heatmap hover", function() {
  ...
  function _hover(gd, xval, yval) {
    var fullLayout = gd._fullLayout, calcData = gd.calcdata, hoverData = [];
