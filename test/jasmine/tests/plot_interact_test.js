...
describe("Test plot structure", function() {
  ...
  function assertNamespaces(node) {
    expect(node.getAttribute("xmlns")).toEqual("http://www.w3.org/2000/svg");
    expect(node.getAttribute("xmlns:xlink")).toEqual(
  ...
  describe("cartesian plots", function() {
      ...
      it("should restore layout axes when they get deleted", function(done) {
        ...
        Plotly.relayout(gd, { xaxis: null, yaxis: null })
          .then(function() {
            ...
            expect(countSubplots()).toEqual(1);
            expect(gd.layout.xaxis.range).toBeCloseToArray(
              [-4.79980, 74.48580],
              4
            );
            expect(gd.layout.yaxis.range).toBeCloseToArray(
              [-1.2662, 17.67023],
              4
            );
    ...
    describe("contour/heatmap traces", function() {
      ...
      function extendMock() {
        ...
        return { data: mockData, layout: mockLayout };
      }
      ...
      describe("structure after restyle", function() {
        beforeEach(function(done) {
          ...
          Plotly.restyle(gd, "type", "heatmap", 2).then(done);
        });
    ...
    describe("pie traces", function() {
      ...
      it("should be able to be restyled to a bar chart and back", function(
        done
      ) {
        expect(countPieTraces()).toEqual(1);
  ...
  describe("geo plots", function() {
    ...
    it(
      "has as many *choroplethlocation* nodes as there are choropleth locations",
      function() {
...
describe("plot svg clip paths", function() {
  ...
  function plot() {
    return Plotly.plot(
      createGraphDiv(),
      [
        { type: "contour", z: [[1, 2, 3], [2, 3, 1]] },
        { type: "scatter", y: [2, 1, 2] }
      ],
      {
        showlegend: true,
        xaxis: { rangeslider: {} },
        shapes: [{ xref: "x", yref: "y", x0: 0, y0: 0, x1: 3, y1: 3 }]
      }
    );
  }
  ...
  it("should set clip path url to ids appended to window url", function(done) {
    ...
    var base = d3.select("body").append("base").attr("href", "https://plot.ly");
