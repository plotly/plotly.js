...
describe("Test lib.js:", function() {
  ...
  describe("interp() should", function() {
    it("return 1.75 as Q1 of [1, 2, 3, 4, 5]:", function() {
      var input = [1, 2, 3, 4, 5], res = Lib.interp(input, 0.25), res0 = 1.75;
      expect(res).toEqual(res0);
    });
    it("return 4.25 as Q3 of [1, 2, 3, 4, 5]:", function() {
      var input = [1, 2, 3, 4, 5], res = Lib.interp(input, 0.75), res0 = 4.25;
      expect(res).toEqual(res0);
    });
    ...
    it("error if second input argument is a date:", function() {
      var in1 = [1, 2, 3, 4, 5], in2 = new Date(2014, 11, 1);
      expect(function() {
        Lib.interp(in1, in2);
      }).toThrow("n should be a finite number");
    });
  ...
  describe("dot()", function() {
    ...
    it("should dot two matrices to a matrix", function() {
      expect(dot([[1, 2], [3, 4]], [[5, 6], [7, 8]])).toEqual([
        [19, 22],
        [43, 50]
      ]);
    });
  ...
  describe("variance() should", function() {
    it("return 0 on input [2, 2, 2, 2, 2]:", function() {
      var input = [2, 2, 2, 2], res = Lib.variance(input);
      expect(res).toEqual(0);
    });
    it("return 2/3 on input [-1, 0, 1]:", function() {
      var input = [-1, 0, 1], res = Lib.variance(input);
      expect(res).toEqual(2 / 3);
    });
    it("toss out non-numerics (strings):", function() {
      var input = [1, 2, "apple", "orange"], res = Lib.variance(input);
      expect(res).toEqual(0.25);
    });
    it("toss out non-numerics (NaN):", function() {
      var input = [1, 2, NaN], res = Lib.variance(input);
      expect(res).toEqual(0.25);
    });
  });
  ...
  describe("stdev() should", function() {
    it("return 0 on input [2, 2, 2, 2, 2]:", function() {
      var input = [2, 2, 2, 2], res = Lib.stdev(input);
      expect(res).toEqual(0);
    });
    it("return sqrt(2/3) on input [-1, 0, 1]:", function() {
      var input = [-1, 0, 1], res = Lib.stdev(input);
      expect(res).toEqual(Math.sqrt(2 / 3));
    });
    it("toss out non-numerics (strings):", function() {
      var input = [1, 2, "apple", "orange"], res = Lib.stdev(input);
      expect(res).toEqual(0.5);
    });
    it("toss out non-numerics (NaN):", function() {
      var input = [1, 2, NaN], res = Lib.stdev(input);
      expect(res).toEqual(0.5);
    });
  });
  ...
  describe("smooth()", function() {
    it("should not alter the input for FWHM < 1.5", function() {
      var input = [1, 2, 1, 2, 1], output = Lib.smooth(input.slice(), 1.49);
    ...
    it(
      "should preserve the length and integral even with multiple bounces",
      function() {
  ...
  describe("nestedProperty", function() {
    ...
    it("should access simple objects", function() {
      var obj = { a: "b", c: "d" }, propA = np(obj, "a"), propB = np(obj, "b");
    ...
    it("should access arrays", function() {
      var arr = [1, 2, 3], prop1 = np(arr, 1), prop5 = np(arr, "5");
    ...
    it(
      "should access properties of objects in an array with index -1",
      function() {
    ...
    it("should remove a property only with undefined or null", function() {
      var obj = { a: "b", c: "d" }, propA = np(obj, "a"), propC = np(obj, "c");
    ...
    it(
      "should have no empty object sub-containers but contain empty data arrays",
      function() {
        var obj = {}, prop = np(obj, "a[1].b.c"), expectedArr = [];
    ...
    it(
      "should get empty, and fail on set, with a bad input object",
      function() {
        var badProps = [
          np(5, "a"),
          np(undefined, "a"),
    ...
    it("should fail on a bad property string", function() {
      var badStr = [[], {}, false, undefined, null, NaN, Infinity];
  ...
  describe("expandObjectPaths", function() {
    ...
    it("unpacks recursively", function() {
      ...
      var expected = {
        marker: { color: { red: { certainty: "definitely" } } }
      };
      expect(Lib.expandObjectPaths(input)).toLooseDeepEqual(expected);
    });
    ...
    it("expands array containers", function() {
      var input = {
        title: "clear annotation 1",
        "annotations[1]": { title: "new" }
      };
      var expected = {
        title: "clear annotation 1",
        annotations: [null, { title: "new" }]
      };
      var computed = Lib.expandObjectPaths(input);
      expect(computed).toLooseDeepEqual(expected);
    });
  ...
  describe("coerce", function() {
    ...
    describe("string valType", function() {
      ...
      it(
        "should insert the default if input is missing, or blank with noBlank",
        function() {
      ...
      it("should always return a string for any input", function() {
        expect(coerce({ s: "a string!!" }, {}, stringAttrs, "s")).toEqual(
          "a string!!"
        );
    ...
    describe("coerce2", function() {
      ...
      it(
        "should set a value and return the value it sets when user input is valid",
        function() {
          var colVal = "red",
            sizeVal = 0,
            // 0 is valid but falsey
            attrs = {
              testMarker: {
      ...
      it(
        "should set and return the default if the user input is not valid",
        function() {
          var colVal = "r",
            sizeVal = "aaaaah!",
            attrs = {
    ...
    describe("info_array valType", function() {
      ...
      it("should insert the default if input is missing", function() {
        ...
        expect(
          coerce(undefined, {}, infoArrayAttrs, "domain", [0, 0.5])
        ).toEqual([0, 0.5]);
      });

      it("should dive into the items and coerce accordingly", function() {
        expect(
          coerce({ range: ["-10", 100] }, {}, infoArrayAttrs, "range")
        ).toEqual([-10, 100]);

        expect(
          coerce({ domain: [0, 0.5] }, {}, infoArrayAttrs, "domain")
        ).toEqual([0, 0.5]);

        expect(
          coerce({ domain: [-5, 0.5] }, {}, infoArrayAttrs, "domain")
        ).toEqual([0, 0.5]);

        expect(
          coerce({ domain: [0.5, 4.5] }, {}, infoArrayAttrs, "domain")
        ).toEqual([0.5, 1]);
      });

      it("should coerce unexpected input as best as it can", function() {
        expect(coerce({ range: [12] }, {}, infoArrayAttrs, "range")).toEqual([
          12
        ]);

        expect(
          coerce({ range: [12] }, {}, infoArrayAttrs, "range", [-1, 20])
        ).toEqual([12, 20]);

        expect(
          coerce({ domain: [0.5] }, {}, infoArrayAttrs, "domain")
        ).toEqual([0.5, 1]);

        expect(
          coerce({ range: ["-10", 100, 12] }, {}, infoArrayAttrs, "range")
        ).toEqual([-10, 100]);

        expect(
          coerce({ domain: [0, 0.5, 1] }, {}, infoArrayAttrs, "domain")
        ).toEqual([0, 0.5]);
      });
    });
  ...
  describe("coerceFont", function() {
    ...
    it("should insert the full default if no or empty input", function() {
      ...
      containerIn = {};
      expect(coerceFont(coerce, "fontNoDefault", defaultFont)).toEqual(
        defaultFont
      );
    ...
    it("should pass through individual valid pieces", function() {
      var goodFamily = "A fish",
        // for now any non-blank string is OK
        badFamily = 42,
        goodSize = 123.456,
        badSize = "ginormous",
        goodColor = "red",
  ...
  describe("validate", function() {
    ...
    it("should work for valType 'data_array' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "data_array" });

      assert(shouldPass, shouldFail, { valType: "data_array", dflt: [1, 2] });
    });
    ...
    it("should work for valType 'boolean' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "boolean", dflt: true });

      assert(shouldPass, shouldFail, { valType: "boolean", dflt: false });
    });
    ...
    it("should work for valType 'number' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "number" });

      assert(shouldPass, shouldFail, { valType: "number", dflt: null });
    ...
    it("should work for valType 'integer' where", function() {
      assert(
        [1, 2, "3", "4"],
        ["a", 1.321321, {}, [], null, 2 / 3, undefined, null],
        { valType: "integer", dflt: 1 }
      );
    ...
    it("should work for valType 'string' where", function() {
      ...
      assert(
        ["3", "4", "a", 3, 1.2113, ""],
        [undefined, {}, [], null, date, false],
        { valType: "string", dflt: "a" }
      );

      assert(
        ["3", "4", "a", 3, 1.2113],
        ["", undefined, {}, [], null, date, true],
        { valType: "string", dflt: "a", noBlank: true }
      );
    ...
    it("should work for valType 'color' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "color" });
    });
    ...
    it("should work for valType 'angle' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "angle", dflt: 0 });
    });
    ...
    it("should work for valType 'subplotid' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "subplotid", dflt: "sp" });
    });
    ...
    it("should work for valType 'any' where", function() {
      ...
      assert(shouldPass, shouldFail, { valType: "any" });
    });
    ...
    it("should work for valType 'info_array' (freeLength case)", function() {
      ...
      assert(shouldPass, shouldFail, {
        valType: "info_array",
        freeLength: true,
        items: [{ valType: "any" }, { valType: "any" }, { valType: "number" }]
      });
    });
  });
  ...
  describe("setCursor", function() {
    ...
    it(
      "should assign cursor- class while present non-cursor- classes",
      function() {
  ...
  describe("overrideCursor", function() {
    ...
    it(
      "should apply the new cursor(s) and revert to the original when removed",
      function() {
        this.el3
        ...
        expect(this.el3.attr("class")).toBe(
          "not-a-cursor another cursor-after"
        );

        overrideCursor(this.el3, "later");
        expect(this.el3.attr("class")).toBe(
          "not-a-cursor another cursor-later"
        );

        overrideCursor(this.el3);
        expect(this.el3.attr("class")).toBe(
          "not-a-cursor another cursor-before"
        );
      }
    );

    it(
      "should apply the new cursor(s) and revert to the none when removed",
      function() {
    ...
    it(
      "should do nothing if no existing or new override is present",
      function() {
  ...
  describe("numSeparate", function() {
    ...
    it(
      "should work even for 4-digit integer if third argument is true",
      function() {
  ...
  describe("cleanNumber", function() {
    ...
    it(
      "should accept number strings with arbitrary cruft on the outside",
      function() {
        [
          ["0", 0],
          ["1", 1],
  ...
  describe("loggers", function() {
    var stashConsole, stashLogLevel;
...
describe("Queue", function() {
  ...
  it(
    "should fill in undoQueue up to value found in *queueLength* config",
    function(done) {
      ...
      Plotly.plot(gd, [{ y: [2, 1, 2] }])
        ...
        .then(function() {
          ...
          expect(
            gd.undoQueue.queue[0].undo.args[0][1]["marker.color"]
          ).toEqual([undefined]);
          expect(gd.undoQueue.queue[0].redo.args[0][1]["marker.color"]).toEqual(
            "red"
          );
        ...
        .then(function() {
          ...
          expect(gd.undoQueue.queue[1].undo.args[0][1].title).toEqual(
            undefined
          );
          expect(gd.undoQueue.queue[1].redo.args[0][1].title).toEqual(
            "A title"
          );
        ...
        .then(function() {
          ...
          expect(gd.undoQueue.queue[1].undo.args[0][1].mode).toEqual([
            undefined
          ]);
          expect(gd.undoQueue.queue[1].redo.args[0][1].mode).toEqual("markers");

          expect(gd.undoQueue.queue[0].undo.args[0][1].title).toEqual(
            undefined
          );
          expect(gd.undoQueue.queue[0].redo.args[0][1].title).toEqual(
            "A title"
          );
        ...
        .then(function() {
          expect(gd.undoQueue.queue[1].undo.args[0][1]).toEqual({
            "transforms[0]": null
          });
          expect(gd.undoQueue.queue[1].redo.args[0][1]).toEqual({
            "transforms[0]": { type: "filter" }
          });
