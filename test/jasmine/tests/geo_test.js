  ...
  describe("supplyLayoutDefaults", function() {
    ...
    it(
      ...
      function() {
        ...
        expectedLonaxisRange = [-255, 105];
        // => -75 +/- 180
        expectedLataxisRange = [-45, 135];
        // => 45 +/- 90
        testOne();
        ...
        geoLayoutOut = {
          scope: "world",
          projection: { type: "orthographic", rotation: { lon: -75, lat: 45 } }
        };
        expectedLonaxisRange = [-165, 15];
        // => -75 +/- 90
        expectedLataxisRange = [-45, 135];
        // => 45 +/- 90
        testOne();
...
describe("Test Geo layout defaults", function() {
  ...
  describe("supplyLayoutDefaults", function() {
    ...
    it(
      "should not coerce projection.rotation if type is albers usa",
      function() {
        layoutIn = {
          geo: {
            projection: { type: "albers usa", rotation: { lon: 10, lat: 10 } }
          }
        };
    ...
    it(
      "should not coerce projection.rotation if type is albers usa (converse)",
      function() {
        layoutIn = { geo: { projection: { rotation: { lon: 10, lat: 10 } } } };
    ...
    it(
      "should not coerce coastlines and ocean if type is albers usa",
      function() {
        layoutIn = {
          geo: {
    ...
    it(
      "should not coerce coastlines and ocean if type is albers usa (converse)",
      function() {
        layoutIn = { geo: { showcoastlines: true, showocean: true } };
    ...
    it("should not coerce projection.parallels if type is conic", function() {
      ...
      function testOne(projType) {
        layoutIn = {
          geo: { projection: { type: projType, parallels: [10, 10] } }
        };
    ...
    it("should coerce subunits only when available (NA case)", function() {
      layoutIn = { geo: { scope: "north america", resolution: 50 } };
    ...
    it("should coerce subunits only when available (NA case 2)", function() {
      layoutIn = { geo: { scope: "north america" } };
...
describe("geojson / topojson utils", function() {
  ...
  function _locationToFeature(topojson, loc, locationmode) {
    ...
    var feature = geoLocationUtils.locationToFeature(
      locationmode,
      loc,
      features
    );
  ...
  describe("mock geo_first.json", function() {
    ...
    describe("scattergeo hover labels", function() {
      ...
      it("should show longitude and latitude values", function() {
        ...
        expect(node.innerHTML).toEqual("(0\xB0, 0\xB0)");
      });
    ...
    describe("scattergeo hover events", function() {
      ...
      it(
        "should not be triggered when pt over on the other side of the globe",
        function(done) {
          var update = {
      ...
      it(
        "should not be triggered when pt *location* does not have matching feature",
        function(done) {
          var update = { locations: [["CAN", "AAA", "USA"]] };
    ...
    describe("streaming calls", function() {
      ...
      function countScatterGeoTextGroups() {
        return d3.selectAll("g.trace.scattergeo").selectAll("g").size();
      }
      ...
      it(
        "should be able to delete line/marker/text nodes and choropleth paths",
        function(done) {
      ...
      it(
        "should be able to update line/marker/text nodes and choropleth paths",
        function(done) {
          var trace0 = gd.data[0];
          trace0.lon = lonQueue;
          trace0.lat = latQueue;
