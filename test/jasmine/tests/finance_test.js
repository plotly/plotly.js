...
describe("finance charts defaults:", function() {
  ...
  it(
    "should slice data array according to minimum supplied length",
    function() {
  ...
  it(
    "should set visible to *false* when minimum supplied length is 0",
    function() {
      var trace0 = Lib.extendDeep({}, mock0, { type: "ohlc" });
  ...
  it(
    "direction *showlegend* should be inherited from trace-wide *showlegend*",
    function() {
  ...
  it("direction *name* should be inherited from trace-wide *name*", function() {
    var trace0 = Lib.extendDeep({}, mock0, { type: "ohlc", name: "Company A" });
  ...
  it(
    "trace *name* default should make reference to user data trace indices",
    function() {
      var trace0 = Lib.extendDeep({}, mock0, { type: "ohlc" });

      var trace1 = { type: "scatter" };

      var trace2 = Lib.extendDeep({}, mock1, { type: "candlestick" });
  ...
  it(
    "trace-wide styling should set default for corresponding per-direction styling",
    function() {
      ...
      var fullInput = fullData.map(function(fullTrace) {
        return fullTrace._fullInput;
      });
  ...
  it("should add a few layout settings by default", function() {
    ...
    var trace1 = Lib.extendDeep({}, mock0, { type: "candlestick" });

    var layout1 = { xaxis: { rangeslider: { visible: false } } };
  ...
  it(
    "should make empty candlestick traces autotype to *linear* (as opposed to real box traces)",
    function() {
...
describe("finance charts calc transforms:", function() {
  ...
  it("should fill *text* for OHLC hover labels", function() {
    ...
    var trace3 = Lib.extendDeep({}, mock0, { type: "ohlc", hoverinfo: "x" });

    var out = _calc([trace0, trace1, trace2, trace3]);

    expect(out[0].hoverinfo).toEqual("x+text+name");
    expect(out[0].text[0]).toEqual(
      "Open: 33.01<br>High: 34.2<br>Low: 31.7<br>Close: 34.1<br>A"
    );
    expect(out[0].hoverinfo).toEqual("x+text+name");
    expect(out[1].text[0]).toEqual(
      "Open: 33.31<br>High: 34.37<br>Low: 30.75<br>Close: 31.93<br>B"
    );
    ...
    expect(out[4].text[0]).toEqual(
      "Open: 33.01<br>High: 34.2<br>Low: 31.7<br>Close: 34.1"
    );
    expect(out[5].hoverinfo).toEqual("text");
    expect(out[5].text[0]).toEqual(
      "Open: 33.31<br>High: 34.37<br>Low: 30.75<br>Close: 31.93"
    );
  ...
  it("should work with *filter* transforms", function() {
    var trace0 = Lib.extendDeep({}, mock1, {
      ...
      tickwidth: 0.05,
      transforms: [
        { type: "filter", operation: ">", target: "open", value: 33 }
      ]
    });
  ...
  it("should work with *groupby* transforms (ohlc)", function() {
    var opts = { type: "groupby", groups: ["b", "b", "b", "a"] };
    ...
    expect(out[0].y).toEqual([33.01, 33.01, 34.2, 31.7, 34.1, 34.1, null]);
    ...
    expect(out[2].y).toEqual([32.06, 32.06, 34.25, 31.62, 33.18, 33.18, null]);
  ...
  it("should work with *groupby* transforms (candlestick)", function() {
    var opts = { type: "groupby", groups: ["a", "b", "b", "a"] };
  ...
  it(
    "should use the smallest trace minimum x difference to convert *tickwidth* to data coords for all traces attached to a given x-axis",
    function() {
      var trace0 = Lib.extendDeep({}, mock1, { type: "ohlc", tickwidth: 0.5 });

      var trace1 = Lib.extendDeep({}, mock1, { type: "ohlc", tickwidth: 0.5 });
  ...
  it(
    "should fallback to a minimum x difference of 0.5 in one-item traces",
    function() {
      var trace0 = Lib.extendDeep({}, mock1, { type: "ohlc", tickwidth: 0.5 });
      trace0.x = ["2016-01-01"];

      var trace1 = Lib.extendDeep({}, mock0, { type: "ohlc", tickwidth: 0.5 });
      trace1.x = [10];
...
describe("finance charts updates:", function() {
  ...
  it("Plotly.restyle should work", function(done) {
    ...
    Plotly.plot(gd, [trace0])
      ...
      .then(function() {
        ...
        return Plotly.restyle(gd, "open", [
          [0, 30.75, 32.87, 31.62, 30.81, 32.75, 32.75, 32.87]
        ]);
      })
      .then(function() {
  ...
  it(
    "Plotly.addTraces + Plotly.relayout should update candlestick box position values",
    function(done) {
  ...
  it(
    "Plotly.plot with data-less trace and adding with Plotly.restyle",
    function(done) {
      var data = [
        { type: "candlestick" },
        { type: "ohlc" },
