...
var mock0 = {
    open: [33.01, 33.31, 33.5, 32.06, 34.12, 33.05, 33.31, 33.5],
    high: [34.2, 34.37, 33.62, 34.25, 35.18, 33.25, 35.37, 34.62],
    low: [31.7, 30.75, 32.87, 31.62, 30.81, 32.75, 32.75, 32.87],
    close: [34.1, 31.93, 33.37, 33.18, 31.18, 33.1, 32.93, 33.7]
};

var mock1 = Lib.extendDeep({}, mock0, {
    x: ['2016-09-01', '2016-09-02', '2016-09-03', '2016-09-04', '2016-09-05', '2016-09-06', '2016-09-07', '2016-09-10']
});
...
describe('finance charts defaults:', function () {
    ...
    it('should not slice data arrays but record minimum supplied length', function () {
        ...
        trace0.open = [33.01, 33.31, 33.5, 32.06, 34.12];
    ...
    it('trace *name* default should make reference to user data trace indices', function () {
        ...
        expect(names).toEqual(['trace 0', 'trace 1', 'trace 2', 'trace 3']);
    });
...
describe('finance charts calc', function () {
    ...
    it("should handle cases where 'open' and 'close' entries are equal", function () {
        ...
        expect(mapGet(out[1], 'dir')).toEqual(['increasing', 'decreasing', 'decreasing', 'increasing']);
    });
...
describe('finance charts updates:', function () {
    ...
    it('should be able to toggle visibility', function (done) {
        var data = [Lib.extendDeep({}, mock0, { type: 'ohlc' }), Lib.extendDeep({}, mock0, { type: 'candlestick' })];
    ...
    it('Plotly.newPlot with data-less trace and adding with Plotly.restyle', function (done) {
        var data = [{ type: 'candlestick' }, { type: 'ohlc' }, { type: 'bar', y: [2, 1, 2] }];
    ...
    it('should be able to update ohlc tickwidth', function (done) {
        ...
        function _assert(msg, exp) {
            ...
            expect(tickLen).toBe(exp.tickLen, 'tickLen val in calcdata - ' + msg);
            var pathd = d3Select(gd).select('.ohlc > path').attr('d');
            expect(pathd).toBe(exp.pathd, 'path d attr - ' + msg);
        }
    ...
    it('should work with typed array', function (done) {
        ...
        var data0 = [Lib.extendDeep({}, mock0, { type: 'ohlc' }), Lib.extendDeep({}, mock0, { type: 'candlestick' })];
    ...
    it('should clear empty candlestick boxes using react', function (done) {
        ...
        Plotly.newPlot(gd, steps[0])
            ...
            .then(function () {
                expect(
                    d3Select('g.cartesianlayer').selectAll('g.trace.boxes').selectAll('path').node().getAttribute('d')
                ).toEqual('M0,0Z');
            })
            .then(done, done.fail);
    });
...
describe('finance charts *special* handlers:', function () {
    ...
    it('`editable: true` handlers should work', function (done) {
        ...
        Plotly.newPlot(
            gd,
            [Lib.extendDeep({}, mock0, { type: 'ohlc' }), Lib.extendDeep({}, mock0, { type: 'candlestick' })],
            {},
            {
                editable: true
            }
