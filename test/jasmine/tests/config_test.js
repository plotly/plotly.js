...
describe("config argument", function() {
  describe("attribute layout.autosize", function() {
    ...
    function testAutosize(
      ...
      var layout = { autosize: autosize, width: layoutWidth },
        relayout = { width: relayoutWidth };
    ...
    it(
      "should fill the frame when autosize: false, fillFrame: true, frameMargins: undefined",
      function(done) {
        var autosize = false,
    ...
    it(
      "should fill the frame when autosize: true, fillFrame: true and frameMargins: undefined",
      function(done) {
        var autosize = true,
    ...
    it(
      "should fill the container when autosize: false, fillFrame: false and frameMargins: undefined",
      function(done) {
        var autosize = false,
          config = { autosizable: true, fillFrame: false },
    ...
    it(
      "should fill the container when autosize: true, fillFrame: false and frameMargins: undefined",
      function(done) {
        var autosize = true,
          config = { fillFrame: false },
          layoutHeight = containerHeightBeforePlot,
    ...
    it(
      "should fill the container when autosize: false, fillFrame: false and frameMargins: 0.1",
      function(done) {
        var autosize = false,
          config = { autosizable: true, fillFrame: false, frameMargins: 0.1 },
          layoutHeight = 360,
          relayoutHeight = layoutHeight;
        testAutosize(autosize, config, layoutHeight, relayoutHeight, done);
      }
    ...
    it(
      "should fill the container when autosize: true, fillFrame: false and frameMargins: 0.1",
      function(done) {
        var autosize = true,
          config = { fillFrame: false, frameMargins: 0.1 },
          layoutHeight = 360,
          relayoutHeight = 288;
        testAutosize(autosize, config, layoutHeight, relayoutHeight, done);
      }
    ...
    it("should respect attribute autosizable: false", function(done) {
      var autosize = false,
        config = { autosizable: false, fillFrame: true },
  ...
  describe("showLink attribute", function() {
    ...
    it("should display a link when true", function() {
      ...
      expect(link.textContent).toBe("Edit chart \xBB");
  ...
  describe("editable attribute", function() {
    ...
    beforeEach(function(done) {
      ...
      Plotly.plot(
        gd,
        [{ x: [1, 2, 3], y: [1, 2, 3] }, { x: [1, 2, 3], y: [3, 2, 1] }],
        {
          width: 600,
          height: 400,
          annotations: [{ text: "testing", x: 1, y: 1, showarrow: true }]
        },
        { editable: true }
      ).then(done);
    });
    ...
    function checkIfEditable(elClass, text) {
      ...
      var editBox = document.getElementsByClassName("plugin-editable editable")[
        0
      ];
