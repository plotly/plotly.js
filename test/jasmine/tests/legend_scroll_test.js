...
describe('The legend', function() {
  ...
  function getPlotHeight(gd) {
    return (
      gd._fullLayout.height - gd._fullLayout.margin.t - gd._fullLayout.margin.b
    );
  }
  ...
  describe('when plotted with many traces', function() {
    ...
    it("should scroll when there's a wheel event", function() {
      var legend = getLegend(),
        ...
        finalDataScroll =
          '' +
          Lib.constrain(
            initialDataScroll - wheelDeltaY / scrollBarYMax * scrollBoxYMax,
            -scrollBoxYMax,
            0
          );
    ...
    it('should be restored and functional after relayout', function(done) {
      ...
      (scrollBarX = scrollBar.getAttribute(
        'x'
    ...
    it('should constrain scrolling to the contents', function() {
      var legend = getLegend(), scrollBox = getScrollBox();
    ...
    it("should be removed from DOM when 'showlegend' is relayout'ed to false", function(
      done
    ) {
