...
describe('Test icicle defaults:', function () {
    ...
    it('should use *layout.colorway* as dflt for *iciclecolorway*', function () {
        ...
        expect(gd._fullLayout.iciclecolorway).toEqual(['red', 'blue', 'green'], 'dflt to layout colorway');

        _supply([{ labels: [1], parents: [''] }], {
            colorway: ['red', 'blue', 'green'],
            iciclecolorway: ['cyan', 'yellow', 'black']
        });
        expect(gd._fullLayout.iciclecolorway).toEqual(['cyan', 'yellow', 'black'], 'user-defined value');
    });
...
describe('Test icicle calc:', function () {
    ...
    it('should warn when there are multiple implied roots', function () {
        ...
        expect(Lib.warn).toHaveBeenCalledWith(
            'Multiple implied roots, cannot build icicle hierarchy of trace 0. These roots include: Root1, Root22'
        );
    });
