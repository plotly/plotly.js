...
describe("scattermapbox defaults", function() {
  ...
  it("should truncate 'lon' if longer than 'lat'", function() {
    var fullTrace = _supply({ lon: [1, 2, 3], lat: [2, 3] });
  ...
  it("should truncate 'lat' if longer than 'lon'", function() {
    var fullTrace = _supply({ lon: [1, 2, 3], lat: [2, 3, 3, 5] });
  ...
  it(
    "should set 'visible' to false if 'lat' and/or 'lon' has zero length",
    function() {
      var fullTrace = _supply({ lon: [1, 2, 3], lat: [] });

      expect(fullTrace.visible).toEqual(false);

      fullTrace = _supply({ lon: null, lat: [1, 2, 3] });
  ...
  it(
    "should set 'marker.color' and 'marker.size' to first item if symbol is set to 'circle'",
    function() {
      var base = {
        mode: "markers",
        lon: [1, 2, 3],
        lat: [2, 3, 3],
        marker: { color: ["red", "green", "blue"], size: [10, 20, 30] }
      };

      var fullTrace = _supply(
        Lib.extendDeep({}, base, { marker: { symbol: "monument" } })
      );
      ...
      fullTrace = _supply(
        Lib.extendDeep({}, base, { marker: { symbol: "circle" } })
      );
...
describe("scattermapbox calc", function() {
  ...
  it("should place lon/lat data in lonlat pairs", function() {
    var calcTrace = _calc(
      Lib.extendFlat({}, base, { lon: [10, 20, 30], lat: [20, 30, 10] })
    );
  ...
  it("should fill marker attributes (symbol case)", function() {
    var calcTrace = _calc(
      Lib.extendFlat({}, base, {
        lon: [10, 20, null, 30],
        lat: [20, 30, null, 10],
        marker: { symbol: ["monument", "music", "harbor", null] }
      })
    );
...
describe("scattermapbox convert", function() {
    ...
    expect(opts.circle.paint["circle-color"]).toEqual(
      {
        ...
        stops: [[0, "rgb(220, 220, 220)"], [1, "#444"], [2, "rgb(178, 10, 28)"]]
      },
      "have correct circle-color stops"
    );

    expect(opts.circle.paint["circle-radius"]).toEqual(
      { property: "circle-radius", stops: [[0, 5], [1, 10], [2, 0]] },
  ...
  it("fill + markers + lines traces, should", function() {
    ...
    expect(opts.line.geojson.coordinates).toEqual(
      lineCoords,
      "have correct line coords"
    );
    expect(opts.fill.geojson.coordinates).toEqual(
      fillCoords,
      "have correct fill coords"
    );
  ...
  it("for text + lines traces, should", function() {
    ...
    var lineCoords = [[10, 20], [20, 20], [30, 10], [20, 10], [10, 20]];
  ...
  it(
    "should correctly convert 'textposition' to 'text-anchor' and 'text-offset'",
    function() {
      var specs = {
  ...
  it(
    "for markers + circle bubbles traces with repeated values, should",
    function() {
      var opts = _convert(
        Lib.extendFlat({}, base, {
      ...
      expect(opts.circle.paint["circle-radius"].stops).toBeCloseTo2DArray(
        [[0, 2.5], [1, 24.5]],
        "not replicate stops"
      );
      ...
      expect(radii).toBeCloseToArray(
        [0, 1, 0],
        "link features to correct stops"
      );
    }
  );
...
describe("scattermapbox hover", function() {
  ...
  function getPointData(gd) {
    var cd = gd.calcdata, mapbox = gd._fullLayout.mapbox._subplot;
  ...
  it("should generate hover label info (hoverinfo: 'lon' case)", function(
    done
  ) {
    Plotly.restyle(gd, "hoverinfo", "lon").then(function() {
      ...
      expect(out.extraText).toEqual("lon: 10\xB0");
      done();
    });
  });

  it("should generate hover label info (hoverinfo: 'lat' case)", function(
    done
  ) {
    Plotly.restyle(gd, "hoverinfo", "lat").then(function() {
      ...
      expect(out.extraText).toEqual("lat: 10\xB0");
      done();
    });
  });

  it("should generate hover label info (hoverinfo: 'text' case)", function(
    done
  ) {
