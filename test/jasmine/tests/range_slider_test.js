...
describe('the range slider', function() {
  ...
  describe('when specified as visible', function() {
    ...
    it('should react to resizing the minimum handle', function(done) {
      var start = 85, end = 140, diff = end - start;

      expect(gd.layout.xaxis.range).toBeCloseToArray([0, 49]);

      slide(start, sliderY, end, sliderY)
        .then(function() {
          var maskMin = children[2], handleMin = children[5];

          expect(gd.layout.xaxis.range).toBeCloseToArray([4, 49], -0.5);
          expect(maskMin.getAttribute('width')).toEqual(String(diff));
          expect(handleMin.getAttribute('transform')).toBe(
            'translate(' + (diff - 2.5) + ',0.5)'
          );
        })
        .then(done);
    });
    ...
    it('should react to resizing the maximum handle', function(done) {
      ...
      slide(start, sliderY, end, sliderY)
        .then(function() {
          var maskMax = children[3], handleMax = children[6];
    ...
    it('should resize the main plot when rangeslider has moved', function(
      ...
      var start = 300,
        ...
        rangeDiff1 =
          gd._fullLayout.xaxis.range[1] - gd._fullLayout.xaxis.range[0],
        rangeDiff2,
        rangeDiff3;
    ...
    it('should relayout with style options', function(done) {
      var bg = children[0], maskMin = children[2], maskMax = children[3];

      var maskMinWidth, maskMaxWidth;

      Plotly.relayout(gd, 'xaxis.range', [5, 10])
        .then(function() {
          (maskMinWidth = +maskMin.getAttribute(
            'width'
    ...
    it('should relayout on size / domain udpate', function(done) {
      var maskMin = children[2], maskMax = children[3];
  ...
  describe('visibility property', function() {
    ...
    it('should add the slider if rangeslider is set to anything', function(
      done
    ) {
      Plotly.plot(gd, [{ x: [1, 2, 3], y: [2, 3, 4] }], {})
        .then(function() {
    ...
    it('should remove the slider if changed to `false` or `undefined`', function(
      done
    ) {
      Plotly.plot(
        gd,
  ...
  describe('handleDefaults function', function() {
    ...
    it('should return early if *visible: false*', function() {
      var layoutIn = {
        xaxis: { rangeslider: { visible: false, range: [10, 20] } },
      },
        layoutOut = { xaxis: { rangeslider: {} } };
  ...
  describe('in general', function() {
    ...
    function assertRange(axRange, rsRange) {
      ...
      expect(gd.layout.xaxis.rangeslider.range).toBeCloseToArray(
        rsRange,
        precision
      );
    }
    ...
    it('should expand its range in accordance with new data arrays', function(
      done
    ) {
      Plotly.plot(
        gd,
    ...
    it('should not expand its range when range slider range is set', function(
      done
    ) {
      Plotly.plot(
        gd,
