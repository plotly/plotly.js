...
var MAPBOX_ACCESS_TOKEN = require(
...
Plotly.setPlotConfig({ mapboxAccessToken: MAPBOX_ACCESS_TOKEN });
...
describe("mapbox defaults", function() {
  ...
  it(
    "should copy ref to input container in full (for updating on map move)",
    function() {
  ...
  it("should accept both string and object style", function() {
    var mapboxStyleJSON = { id: "cdsa213wqdsa", owner: "johnny" };
  ...
  it("should skip over non-object layer containers", function() {
    layoutIn = { mapbox: { layers: [{}, null, "remove", {}] } };
  ...
  it("should coerce 'sourcelayer' only for *vector* 'sourcetype'", function() {
    layoutIn = {
      mapbox: {
        layers: [
          { sourcetype: "vector", sourcelayer: "layer0" },
          { sourcetype: "geojson", sourcelayer: "layer0" }
        ]
      }
    };
  ...
  it("should only coerce relevant layer style attributes", function() {
    ...
    layoutIn = {
      mapbox: {
        layers: [
          Lib.extendFlat({}, base, { type: "line", color: "red" }),
          Lib.extendFlat({}, base, { type: "fill", color: "blue" }),
          Lib.extendFlat({}, base, { type: "circle", color: "green" }),
          Lib.extendFlat({}, base, { type: "symbol", color: "yellow" })
        ]
      }
    };
...
describe("mapbox credentials", function() {
  ...
  it(
    "should throw error if token is not registered",
    function() {
      expect(function() {
        Plotly.plot(gd, [
          { type: "scattermapbox", lon: [10, 20, 30], lat: [10, 20, 30] }
        ]);
  ...
  it(
    "should use access token in mapbox layout options if present",
    function(done) {
      var cnt = 0;
      ...
      Plotly.plot(
        ...
        .then(function() {
          ...
          expect(gd._fullLayout.mapbox.accesstoken).toEqual(
            MAPBOX_ACCESS_TOKEN
          );
          done();
        });
  ...
  it(
    "should bypass access token in mapbox layout options when config points to an Atlas server",
    function(done) {
      var cnt = 0;
      var msg = [
...
describe("mapbox plots", function() {
  ...
  var mock = require("@mocks/mapbox_0.json"), gd;

  var pointPos = [579, 276], blankPos = [650, 120];
  ...
  it(
    "should be able to toggle trace visibility",
    function(done) {
  ...
  it(
    "should be able to delete and add traces",
    function(done) {
      var modes = ["line", "circle"];
  ...
  it(
    ...
    function(done) {
      var restyleCnt = 0, relayoutCnt = 0;
      ...
      assertMarkerColor([[0.121, 0.466, 0.705, 1], [1, 0.498, 0.0549, 1]])
        .then(function() {
        ...
        .then(function() {
          ...
          return assertMarkerColor([[0, 0.5019, 0, 1], [0, 0.5019, 0, 1]]);
        })
        .then(function() {
        ...
        .then(function() {
          ...
          return assertMarkerColor([[0, 0.5019, 0, 1], [1, 0, 0, 1]]);
        })
        .then(done);
    },
    LONG_TIMEOUT_INTERVAL
  ...
  it(
    ...
    function(done) {
      ...
      function assertLayout(style, center, zoom, dims) {
        ...
        expect([mapInfo.center.lng, mapInfo.center.lat]).toBeCloseToArray(
          center
        );
  ...
  it(
    "should be able to add, update and remove layers",
    function(done) {
  ...
  it(
    "should be able to update the access token",
    function(done) {
  ...
  it(
    "should be able to update traces",
    function(done) {
      ...
      var update = { lon: [[10, 20]], lat: [[-45, -20]] };
  ...
  it(
    "should display to hover labels on mouse over",
    function(done) {
  ...
  it(
    "should respond to hover interactions by",
    function(done) {
  ...
  it(
    "should respond drag / scroll interactions",
    function(done) {
      var relayoutCnt = 0, updateData;
      ...
      function _drag(p0, p1, cb) {
        var promise = _mouseEvent("mousemove", p0, noop)
          .then(function() {
      ...
      function assertLayout(center, zoom, opts) {
        var mapInfo = getMapInfo(gd), layout = gd.layout.mapbox;
        ...
        if (opts && opts.withUpdateData) {
          ...
          expect([
            mapboxUpdate.center.lon,
            mapboxUpdate.center.lat
  ...
  it(
    "should respond to click interactions by",
    function(done) {
      var ptData;
      ...
      function _click(pos, cb) {
        var promise = _mouseEvent("mousemove", pos, noop)
          .then(function() {
  ...
  function getMapInfo(gd) {
    var subplot = gd._fullLayout.mapbox._subplot, map = subplot.map;
  ...
  function countVisibleTraces(gd, modes) {
    var mapInfo = getMapInfo(gd), cnts = [];
  ...
  function getStyle(gd, mode, prop) {
    var mapInfo = getMapInfo(gd), values = [];
