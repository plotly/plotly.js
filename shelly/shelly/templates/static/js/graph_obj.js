...
Plotly.plot = function(gd, data, layout) {
    ...
    if(data[0] && data[0].type.substr(0, 5) === 'Polar'){
        Plotly.plotPolar(gd, data, layout);
        return null;
    }
...
Plotly.plotPolar = function(gd, data, layout){

    gd.data=data;
    gd.layout=layout;

    var container = d3.select('.svg-container').html('');
    ...
    function titleLayout(){
        var bg = svg.select('.background-circle');
