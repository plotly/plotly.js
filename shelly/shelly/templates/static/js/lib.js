...
// separate out 2-digit and 4-digit-year formats, formats with month names,
// and formats with am/pm so we can cut down the number of tests we need to run
var timeFormats = {
...
['Y','Yb','y','yb'].forEach(function(dateType) { dateFormats[dateType].forEach(function(dateFormat){
    dateTimeFormats[dateType].D.push(formatter(dateFormat)); // just a date
...
var matchword = /[a-z]*/g,
    ...
    weekdaymatch = /(mon|tue|wed|thu|fri|sat|sun)/g,
    separatormatch = /[\s,\/\-\.\(\)]+/g,
    ampmmatch = /~?([ap])~?m(~|$)/,
...
lib.parseDate = function(v) {
    ...
    v = v.toLowerCase()
        ...
        // collapse all separators one @ at a time,
        // except : which seems pretty consistent for the time part
